...
def login(request):
    ...
    try:
        if res['result']['error_code'] == 0:
            create_session_product(request, 'lab_pintar', 180)
            ...
            _logger.info("SIGNIN LAB PINTAR")
    except Exception as e:
        _logger.error(msg=str(e) + '\n' + traceback.format_exc())
...
def get_config(request):
    try:
        ...
        file = read_cache("lab_pintar_cache_data", 'cache_web', request, 86400)
...
def get_availability(request):
    try:
        additional_url = 'booking/labpintar'
        headers = {
            "Accept": "application/json,text/html,application/xml",
            "Content-Type": "application/json",
        ...
        data = {
            'provider': 'labpintar', #hardcode dulu
...
def booker_insentif_booking(request):
    ...
    url_request = get_url_gateway('booking/lab_pintar')
    res = send_request_api(request, url_request, headers, data, 'POST', 300)
    try:
        if res['result']['error_code'] == 0:
            total_upsell = 0
            ...
            _logger.info("SUCCESS update_service_charge_booker LAB PINTAR SIGNATURE " + request.POST['signature'])
        else:
            _logger.error("ERROR update_service_charge_lab_pintar_booker LAB PINTAR SIGNATURE " + request.POST['signature'] + ' ' + json.dumps(res))
    except Exception as e:
        _logger.error(str(e) + '\n' + traceback.format_exc())
    return res
