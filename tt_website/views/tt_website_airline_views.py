            ...
            try:
                ...
                if advance_search['airline_advance_pax_type'] == 'true':
                    if advance_search['airline_pax_type_student'] == 'true':
                        airline_request['student'] = direction == 'MC' and int(request.POST['student_flight1']) or int(request.POST['student_flight'])
...
def passenger(request, signature):
    if 'user_account' in request.session._session and 'ticketing_airline' in request.session['user_account']['co_agent_frontend_security']:
        ...
        # CHECK INI
        if 'airline_price_itinerary_%s' % signature not in request.session or request.session.get('airline_price_itinerary_%s' % signature) and request.session['airline_price_itinerary_%s' % signature] != json.loads(request.POST['airline_price_itinerary']):
            set_session(request, 'airline_price_itinerary_%s' % signature, json.loads(request.POST['airline_price_itinerary']))

        if 'airline_get_price_request_%s' % signature not in request.session or request.session.get('airline_get_price_request_%s' % signature) and request.session['airline_get_price_request_%s' % signature] != json.loads(request.POST['airline_price_itinerary_request']):
            set_session(request, 'airline_get_price_request_%s' % signature,json.loads(request.POST['airline_price_itinerary_request']))

        if 'airline_sell_journey_%s' % signature not in request.session or request.session.get('airline_sell_journey_%s' % signature) and request.session['airline_sell_journey_%s' % signature] != json.loads(request.POST['airline_sell_journey_response']):
        ...
        try:
            values.update({
                ...
                'price': copy.deepcopy(request.session['airline_sell_journey_%s' % signature]),
                'airline_get_price_request': copy.deepcopy(request.session['airline_get_price_request_%s' % signature]),
                'airline_carriers': carrier,
                'airline_pick': copy.deepcopy(request.session['airline_sell_journey_%s' % signature]['sell_journey_provider']),
                'adults': adult,
                'childs': child,
                'infants': infant,
                'students': student,
    ...
    if 'user_account' in request.session._session and 'ticketing_airline' in request.session['user_account']['co_agent_frontend_security']:
        ...
        try:
            values.update({
                ...
                'is_identity_required': is_identity_required,
                'order_number': copy.deepcopy(request.session['airline_get_booking_response_%s' % signature])['result']['response']['order_number'],
                'airline_request': copy.deepcopy(request.session['airline_request_%s' % signature]),
                'airline_carriers': carrier,
                'adults': adult,
                'childs': child,
        ...
        try:
            ...
            try:
                ...
                values.update({
                    ...
                    # 'airline_destinations': airline_destinations,
                    'airline_pick': copy.deepcopy(request.session['airline_sell_journey_%s' % signature])['sell_journey_provider'],
                    'upsell': copy.deepcopy(request.session.get('airline_upsell_'+signature)) and request.session.get('airline_upsell_'+signature) or 0,
                    'signature': signature,
                    ...
                    'javascript_version': javascript_version,
                    'static_path_url_server': get_url_static_path(),
                    'time_limit': int(copy.deepcopy(request.session['time_limit_%s' % signature])),
                    'airline_getbooking': ''
                })
            except:
                #dari getbooking
                ...
                values.update({
                    ...
                    'airline_carriers': carrier,
                    'upsell': upsell,
                    'airline_getbooking': copy.deepcopy(request.session['airline_get_booking_response_%s' % signature])['result']['response'],
                    'airline_ssrs': airline_ssr,
