...
def lab_pintar(request):
    if 'user_account' in request.session._session and 'ticketing_labpintar' in request.session['user_account']['co_agent_frontend_security']:
        try:
            ...
            try:
                cache['lab_pintar'] = {
                    'name': request.session['lab_pintar_request']['name'],
                    'date': request.session['lab_pintar_request']['date'],
                }
                if cache['lab_pintar']['date'] == 'Invalid date':
                    cache['lab_pintar']['date'] = convert_string_to_date_to_string_front_end(str(datetime.now())[:10])
            except:
                pass
            values.update({
                ...
                'big_banner_value': check_banner('labpintar', 'big_banner', request),
                'small_banner_value': check_banner('labpintar', 'small_banner', request),
        ...
        return render(request, MODEL_NAME + '/lab_pintar/lab_pintar_templates.html', values)
    else:
...
def passenger(request, test_type=''):
    try:
        ...
        try:
            ...
            set_session(request, 'lab_pintar_passenger_cache', passengers)
        except:
            try:
                passengers = request.session['lab_pintar_passenger_cache']
            except:
                passengers = []

        try:
            booking_data = json.loads(request.POST['booking_data'])
            set_session(request, 'lab_pintar_data_cache', booking_data)
        except:
            pass
        ...
        _logger.error(str(e) + '\n' + traceback.format_exc())
        raise Exception('Make response code 500!')
    return render(request, MODEL_NAME+'/lab_pintar/lab_pintar_passenger_templates.html', values)
    ...
    if 'user_account' in request.session._session:
        try:
            ...
            try:
                ...
                try:
                    data = request.session.get('lab_pintar_data_%s' % request.POST['signature'])
                except:
                    pass
            ...
            lab_pintar_passenger = copy.deepcopy(adult)
            for rec in lab_pintar_passenger:
            ...
            set_session(request, 'lab_pintar_data_%s' % request.POST['signature'], {
                'booker': booker,
                'passengers': adult,
                'contacts': contact,
                'data': data
            ...
            try:
                ...
                set_session(request, 'lab_pintar_signature', request.session['lab_pintar_signature'])
        ...
        try:
            values.update({
                ...
                'upsell': request.session.get('lab_pintar_upsell_' + request.session['lab_pintar_signature']) and request.session.get('lab_pintar_upsell_' + request.session['lab_pintar_signature']) or 0,
                'username': request.session['user_account'],
                'data': request.session['lab_pintar_data_%s' % request.POST['signature']],
                'javascript_version': javascript_version,
                'signature': request.session['lab_pintar_signature'],
                'static_path_url_server': get_url_static_path(),
                'vendor': vendor,
                'test_type': test_type,
            ...
            _logger.error(str(e) + '\n' + traceback.format_exc())
            raise Exception('Make response code 500!')
        return render(request, MODEL_NAME+'/lab_pintar/lab_pintar_review_templates.html', values)
    else:
        return no_session_logout(request)
...
def booking(request, order_number):
    try:
        ...
        if 'user_account' not in request.session and 'btc' in web_mode:
            ...
            raise Exception('Lab Pintar get booking without login in btb web')
        try:
        ...
        try:
            ...
            pass
        try:
            if request.session.get('lab_pintar_data_cache'):
                del request.session['lab_pintar_data_cache']
        except:
            pass
        values.update({
            'static_path': path_util.get_static_path(MODEL_NAME),
            ...
            'order_number': request.session['lab_pintar_order_number'],
        ...
        if 'btc' in web_mode:
            raise Exception('Make response code 500!')
    return render(request, MODEL_NAME+'/lab_pintar/lab_pintar_booking_templates.html', values)
...
def confirm_order(request):
    try:
        ...
        _logger.error(str(e) + '\n' + traceback.format_exc())
        raise Exception('Make response code 500!')
    return render(request, MODEL_NAME+'/lab_pintar/lab_pintar_confirm_order_templates.html', values)
