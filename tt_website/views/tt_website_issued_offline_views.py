...
def booking(request, order_number):
    try:
        ...
        if 'user_account' not in request.session and 'btc' in web_mode:
            ...
            raise Exception('Issued Offline get booking without login in btb web')
        if translation.LANGUAGE_SESSION_KEY in request.session:
            del request.session[translation.LANGUAGE_SESSION_KEY] #get language from browser
        try:
            ...
            try:
                set_session(request, 'issued_offline_order_number', base64.b64decode(order_number[:-1]).decode('ascii'))
            except:
