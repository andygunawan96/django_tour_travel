        ...
        if 'user_account' in request.session._session and 'ticketing_phc' in request.session['user_account']['co_agent_frontend_security'] or \
            ...
            try:
                ...
                values.update({
                    ...
                    'big_banner_value': check_banner(vendor, 'big_banner', request),
                    'small_banner_value': check_banner(vendor, 'small_banner', request),
...
def review(request, vendor):
    if 'user_account' in request.session._session:
        try:
            ...
            try:
                time_limit = get_timelimit_product(request, 'medical')
                if time_limit == 0:
...
def booking(request, vendor, order_number):
    try:
        ...
        if 'user_account' not in request.session and 'btc' in web_mode:
            ...
            raise Exception('Medical (PHC/Periksain) get booking without login in btb web')
        try:
