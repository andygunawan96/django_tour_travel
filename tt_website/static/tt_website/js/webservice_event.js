            ...
            text = `
                    ...
                    for(i in msg.result.response.options){
                        ...
                        text+=`
                            ...
                            js_total_price = 0;
                            js_currency = '';
                            for(x in msg.result.response.passengers[0].sale_service_charges){
                                for(y in msg.result.response.passengers[0].sale_service_charges[x]){
                                    js_currency = msg.result.response.passengers[0].sale_service_charges[x][y].currency
                                    break;
                                }
                                break;
                            }
                    ...
                    if(parseInt(msg.result.response[i].qty_available) >= 1){
                        ...
                        <div class="col-lg-4 mt-1" style="padding:0px; text-align:right;">
                            <div class="row">
                                ...
                                if(typeof(currency_rate_data) !== 'undefined' && currency_rate_data.result.is_show && msg.result.response[i].price){
                                    if(user_login.hasOwnProperty('co_ho_seq_id') && currency_rate_data.result.response.agent.hasOwnProperty(user_login.co_ho_seq_id)){ // buat o3
                                        for(k in currency_rate_data.result.response.agent[user_login.co_ho_seq_id]){
                                            try{
                                                if(currency_rate_data.result.response.agent[user_login.co_ho_seq_id][k].base_currency == msg.result.response[i].currency){
                                                    price_convert = (msg.result.response[i].price/currency_rate_data.result.response.agent[user_login.co_ho_seq_id][k].rate).toFixed(2);
                                                    if(price_convert%1 == 0)
                                                        price_convert = parseInt(price_convert);
                                                    text+=`
                                                        <span class="option_price" style="font-weight: bold; font-size:15px; padding-bottom:10px; padding-top:5px;"> Estimated `+k+` `+price_convert+`</span><br/>`;
                                                }
                                            }catch(err){
                                                console.log(err);
                                            }
...
//                        node.className = 'detail-event-box-sold';
                ...
                if(typeof(currency_rate_data) !== 'undefined' && currency_rate_data.result.is_show && price_start[0]){
                    text_currency = '';
                    if(user_login.hasOwnProperty('co_ho_seq_id') && currency_rate_data.result.response.agent.hasOwnProperty(user_login.co_ho_seq_id)){ // buat o3
                        for(k in currency_rate_data.result.response.agent[user_login.co_ho_seq_id]){
                            try{
                                if(currency_rate_data.result.response.agent[user_login.co_ho_seq_id][k].base_currency == msg.result.response[0].currency){
                                    price_convert = (price_start[0]/currency_rate_data.result.response.agent[j][k].rate).toFixed(2);
                                    if(price_convert%1 == 0)
                                        price_convert = parseInt(price_convert);
                                    text_currency+=`<span style="font-weight:500; font-size:14px;"> Estimated `+k+` `+price_convert+`</span><br/>`;
                                }
                            }catch(err){
                                console.log(err);
                            }
                    ...
                    if(text_currency)
                        document.getElementById('price_start_event_other').innerHTML = text_currency;
            //        }
                }
    ...
    <div class="row">
        <div class="col-lg-12" style="margin-bottom:15px;" id="share_result">
            ...
            if (isMobile) {
                ...
                text+=`
                    <a href="mailto:?subject=This is the event price detail&amp;body=`+ $text_share +`" title="Share by Email" style="padding-right:5px;" target="_blank"><img style="height:30px; width:auto;" src="/static/tt_website/img/email.png" alt="Email"/></a>`;
            } else {
                text+=`
                    <a href="https://web.whatsapp.com/send?text=`+ $text_share +`" data-action="share/whatsapp/share" title="Share by Whatsapp" style="padding-right:5px;" target="_blank"><img style="height:30px; width:auto;" src="/static/tt_website/img/whatsapp.png" alt="Whatsapp"/></a>`;
                if(option_number < 11){
