                        ...
                        <div class="col-lg-6" style="margin-top:15px;">
                            ...
                            text_div_paxs += '<option value="">Choose</option>';
                                for(i in medical_config.result.response.profession)
                                    if(medical_config.result.response.profession[i] == 'SWASTA')
                                        text_div_paxs+=`<option value="`+medical_config.result.response.profession[i]+`" selected>`+medical_config.result.response.profession[i]+`</option>`;
                                    else
                            ...
                            <div class="col-lg-6" style="margin-top:15px;">
                                ...
                                text_div_paxs += '<option value="">Choose</option>';
                                    for(i in medical_config.result.response.perusahaan)
                                        if(medical_config.result.response.perusahaan[i] == 'PRIBADI')
                                            text_div_paxs+=`<option value="`+medical_config.result.response.perusahaan[i]+`" selected>`+medical_config.result.response.perusahaan[i]+`</option>`;
                                        else
                                ...
                                text_div_paxs+=`
                                    <div class="col-lg-6" style="margin-top:15px;">
                                        <div id="suhu_tubuh_div`+parseInt(counter_passenger+1)+`" hidden>
                                            ...
                                            <div class="input-container-search-ticket" style="margin-bottom:5px;">
                                                <input type="number" class="form-control date-picker-birth" name="adult_klinis_suhu_tubuh`+parseInt(counter_passenger+1)+`" id="adult_klinis_suhu_tubuh`+parseInt(counter_passenger+1)+`" placeholder="Suhu Tubuh " onfocus="this.placeholder = ''" onblur="this.placeholder = 'Suhu Tubuh '" autocomplete="off">
                                            </div>
                                        </div>
                                    </div>`;
            ...
            for(i=0; i < counter_passenger; i++){
                ...
                try{
                    ...
                    request['passengers'].push({
                        ...
                        "birth_date": document.getElementById('adult_birth_date' + nomor_pax).value,
                        "identity": {
                            "identity_number": document.getElementById('adult_identity_number' + nomor_pax + '_id').value,
                            "identity_type": document.getElementById('adult_identity_type' + nomor_pax).value,
                            "identity_expdate": document.getElementById('adult_identity_expired_date'+ nomor_pax).value,
                        },
                        "passenger_seq_id": document.getElementById('adult_id' + nomor_pax).value,
    ...
    for(idx in passenger_data_cache_medical){
        if(idx == 0){
            ...
            if(passenger_data_cache_medical[idx].phone_number.substr(0,2) == '62'){
                ...
                $('#booker_phone_code_id').val(passenger_data_cache_medical[idx].phone_number.substr(0,2)).trigger('change');
                document.getElementById('booker_phone').value = passenger_data_cache_medical[idx].phone_number.substr(2,100);
            }
            $('#booker_nationality'+counter+'_id').val(passenger_data_cache_medical[idx].nationality_code).trigger('change');
        }
        if(passenger_data_cache_medical[idx].title == 'MR')
            document.getElementById('adult_title'+counter).value = passenger_data_cache_medical[idx].title;
        else
    ...
    for(idx in passenger_data_cache_medical){
        if(idx == 0){
            ...
            if(passenger_data_cache_medical[idx].phone_number.substr(0,2) == '62'){
                ...
                $('#adult_phone_code_id').val(62).trigger('change');
                document.getElementById('booker_phone').value = passenger_data_cache_medical[idx].phone_number.substr(1,100);
            }else{
                $('#adult_phone_code_id').val(passenger_data_cache_medical[idx].phone_number.substr(0,2)).trigger('change');
                document.getElementById('booker_phone').value = passenger_data_cache_medical[idx].phone_number.substr(2,100);
            }
            $('#booker_nationality_id').val(passenger_data_cache_medical[idx].nationality_code).trigger('change');
        }
