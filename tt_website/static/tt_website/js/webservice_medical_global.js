...
//                                        price['CSC'] = msg.result.response.passengers[j].channel_service_charges.amount;
    ...
    //                            }
        ...
        //                    </div>`;
                            ...
                            }else if( Object.keys(msg.result.response.picked_timeslot).length > 0 && moment().format('YYYY-MM-DD') != msg.result.response.picked_timeslot.datetimeslot.substr(0,10) && window.location.pathname.includes('confirm_order') == true){
                                text_detail+=`
                                <div style="margin-top:10px;">
                                    <center>
...
function medical_global_issued_booking(data){
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        if(document.getElementById('medical_global_payment_form'))
        {
            var formData = new FormData($('#medical_global_payment_form').get(0));
        }
        else
        {
            var formData = new FormData($('#global_payment_form').get(0));
