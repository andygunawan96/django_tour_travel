...
function update_service_charge(type){
    ...
    if(type == 'booking'){
        ...
        for(i in act_get_booking.result.response.passengers){
            ...
            list_price = []
            if(document.getElementById(act_get_booking.result.response.passengers[i].name+'_repricing').innerHTML != '-'){
                list_price.push({
    ...
    //                            }
                    ...
                    price_text+= `
                     ...
                     if(msg.result.response.state == 'booked' && user_login.co_agent_frontend_security.includes('b2c_limitation') == false && user_login.co_agent_frontend_security.includes("corp_limitation") == false)
                        price_text+=`<div style="text-align:right; padding-bottom:10px;"><img src="/static/tt_website/images/icon/symbol/upsell_price.png" alt="Bank" style="width:auto; height:25px; cursor:pointer;" onclick="show_repricing();"/></div>`;
                     ...
                     if(user_login.co_agent_frontend_security.includes('see_commission') && !user_login.co_agent_frontend_security.includes('b2c_limitation') && !user_login.co_agent_frontend_security.includes("corp_limitation")){
                        ...
                        <div class="alert alert-success" style="margin-top:10px;">
                            <div style="color:black; font-weight:bold; cursor:pointer; font-size:15px; text-align:left; width:100%;" onclick="show_commission('show_commission');">
                                ...
                                if(commission == 0){
                                    price_text+=`<span style="font-size:13px; font-weight: bold;color:red">* Please mark up the price first</span>`;
                                }
                                price_text+=`
                                </span>
                            </div>
                    ...
                    if(is_show_breakdown_price && !user_login.co_agent_frontend_security.includes("corp_limitation") && !user_login.co_agent_frontend_security.includes("b2c_limitation")){
                        ...
                        for(i in act_get_booking.result.response.passengers){
                            ...
                            for(j in act_get_booking.result.response.passengers[i].service_charge_details){
                                ...
                                price_breakdown['FARE'] = act_get_booking.result.response.passengers[i].service_charge_details[j].base_fare;
                                ...
                                price_breakdown['OTT'] = act_get_booking.result.response.passengers[i].service_charge_details[j].base_price_ott;
