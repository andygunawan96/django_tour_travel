...
function activity_search(){
    ...
    $.ajax({
       ...
       success: function(msg) {
        if(google_analytics != '')
           ...
           if(msg.result.error_code == 0){
              ...
              <div class="col-lg-12">
                  <div style="text-align:center">
                      <img src="/static/tt_website/images/no_found/no-activity.png" alt="Not Found Activity" style="width:70px; height:70px;" alt="" title="" />
                      <br/>
                  </div>
                  <center><div class="alert alert-warning" role="alert" style="margin-top:15px; border:1px solid #cdcdcd;"><h6><i class="fas fa-search-minus"></i> Oops! Activity not found. Please try again or search another activity. </h6></div></center>
              </div>`;
       ...
       error: function(XMLHttpRequest, textStatus, errorThrown) {
          ...
          <div class="col-lg-12">
              <div style="text-align:center">
                  <img src="/static/tt_website/images/no_found/no-activity.png" style="width:70px; height:70px;" alt="Not Found Activity" title="" />
                  <br/>
              </div>
              <center><div class="alert alert-warning" role="alert" style="margin-top:15px; border:1px solid #cdcdcd;"><h6><i class="fas fa-search-minus"></i> Oops! Activity not found. Please try again or search another activity. </h6></div></center>
          </div>`;
...
function update_service_charge(type){
    ...
    if(type == 'booking'){
        ...
        for(i in act_get_booking.result.response.passengers){
            ...
            list_price = []
            if(document.getElementById(act_get_booking.result.response.passengers[i].name+'_repricing').innerHTML != '-' && document.getElementById(act_get_booking.result.response.passengers[i].name+'_repricing').innerHTML != '0'){
                list_price.push({
                    'amount': parseInt(document.getElementById(act_get_booking.result.response.passengers[i].name+'_repricing').innerHTML.split(',').join('')),
        ...
        upsell = []
        counter_pax = 0;
        currency = price.service_charge_summary[0].service_charges[0].currency;
        for(i in all_pax){
            list_price = [];
    ...
    //                            }
                    ...
                    price_text+= `
                     ...
                     if(msg.result.response.state == 'booked' && user_login.co_agent_frontend_security.includes('b2c_limitation') == false && user_login.co_agent_frontend_security.includes("corp_limitation") == false)
                        price_text+=`<div style="text-align:right; padding-bottom:10px;"><img src="/static/tt_website/img/bank.png" alt="Bank" style="width:auto; height:25px; cursor:pointer;" onclick="show_repricing();"/></div>`;
                        ...
                        price_text+=`<div style="text-align:right; padding-bottom:10px;"><img src="/static/tt_website/img/banknew.png" alt="Bank" style="width:auto; height:25px; cursor:pointer;" onclick="show_repricing();"/></div>`;
...
function upload_image(){
    var formData = new FormData($('#activity_review').get(0));
    formData.append('signature', signature)
    getToken();
    $.ajax({
       type: "POST",
       ...
       success: function(msg) {
            if(msg.result.error_code == 0){
                ...
                for(var i=0;i<senior;i++){
                    ...
                    for(var j=0;j<100;j++){
                        try{
                            if(document.getElementById('senior_identity'+index+'_'+j+'_delete').checked == true)
                                img_list.push([document.getElementById('senior_identity'+index+'_'+j+'_image_seq_id').value, 2, 'senior_files_attachment_identity1']);
                        }catch(err){
                            //gambar habis
                            break;
                        }
