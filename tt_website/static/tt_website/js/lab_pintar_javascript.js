...
function update_timeslot(val){
    ...
    change_timeslot(val);
    document.getElementById('next_lab_pintar').style.display='none';
    try{
        document.getElementById('lab_pintar_pax_div').hidden = true;
    }catch(err){
...
function get_area_global(){
    ...
    if(test_type.includes('PHCHC')){
        ...
        $('html, body').animate({
            scrollTop: $("#div_schedule_lab_pintar").offset().top - 110
        }, 500);
    }
}
...
function change_timeslot(val){
    ...
    document.getElementById('lab_pintar_detail').style.display = 'none';
}
...
function regex_phone_lab_pintar(value){
  var regex_check = "^[1-9][0-9]*$";//number
    if(value.match(regex_check)!=null){
        return value
    }else{
        value = regex_phone_lab_pintar(value.slice(1,value.length));
        return value;
    }
}
...
function onchange_number_no_zero(val){
    if(document.getElementById('adult_phone'+val).value != '' && document.getElementById('adult_phone'+val).value != undefined){
        var data = regex_phone_lab_pintar(document.getElementById('adult_phone'+val).value)
...
//                }
        ...
        if(counter_passenger == 0)
            ...
            for(i=0; i < counter_passenger; i++){
                ...
                try{
                    ...
                    if(check_phone_number(document.getElementById('adult_phone' + nomor_pax + '_id').value)==false){
                        error_log+= 'Phone number only contain number 8 - 12 digits for customer '+nomor_pax+'!</br>\n';
                        document.getElementById('adult_phone' + nomor_pax + '_id').style['border-color'] = 'red';
                        check_form_periksain = 1;
                    }else{
                        document.getElementById('adult_phone' + nomor_pax + '_id').style['border-color'] = '#EFEFEF';
                    }
                    ...
                    if(document.getElementById('adult_identity_type' + nomor_pax).value != ''){
                        ...
                        if(document.getElementById('adult_identity_type' + nomor_pax).value == 'ktp'){
                            ...
                            if(document.getElementById('adult_country_of_issued'+ nomor_pax + '_id').value == ''){
                                error_log+= 'Please fill country of issued for customer '+ nomor_pax +'!</br>\n';
                                document.getElementById('adult_country_of_issued'+ nomor_pax + '_id').style['border-color'] = 'red';
                                check_form_personal = 1;
                            }else{
                                document.getElementById('adult_country_of_issued'+ nomor_pax + '_id').style['border-color'] = '#EFEFEF';
                            }
    ...
    //       re_medical_signin('next');
            ...
            document.getElementById('lab_pintar_review').submit();
       }else{
           ...
           if(check_scroll == "booker"){
                ...
                $('html, body').animate({
                    scrollTop: $("#lab_pintar_pax_div").offset().top - 120
                }, 500);
           }else{
                $('html, body').animate({
...
function add_table(change_rebooking=false){
    ...
    if(change_rebooking == true && total_passengers_rebooking != 0){
        ...
        // isi data passenger
        get_data_cache_passenger_lab_pintar();
        get_data_cache_schedule_lab_pintar();
...
function add_table_verify(change_rebooking=false){
    ...
    if(change_rebooking == true && total_passengers_rebooking != 0){
        ...
        last_counter = tempcounter;
        // isi data
        get_data_cache_passenger_lab_pintar('verify');
    }
    last_counter = tempcounter;
}
...
function next_prev_form_medical(type, counter, id){
    ...
    length_progress = $(".progress_lab_pintar"+pax).length;
...
function auto_fill_data(){
    ...
    for(idx in passenger_data_cache_medical){
        ...
        if(passenger_data_cache_medical[idx].phone_number.substr(0,2) == '62'){
            $('#adult_phone_code'+counter).val(62).trigger('change');
            ...
            $('#adult_phone_code'+counter).val(passenger_data_cache_medical[idx].phone_number.substr(0,2)).trigger('change');
            document.getElementById('adult_phone'+counter).value = passenger_data_cache_medical[idx].phone_number.substr(2,100);
        }
