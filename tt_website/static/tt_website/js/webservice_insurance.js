...
//                        document.getElementById('div_sync_status').hidden = true;
    ...
    //                    </div>`;
                    ...
                    // === Button 2 ===
                        ...
                        print_text+=`
                        <button class="primary-btn hold-seat-booking-train ld-ext-right" id="button-print-itin-price" type="button" onclick="get_printout('` + msg.result.response.order_number + `','itinerary_price','insurance');" style="width:100%;">
                            Print Form (Price)
                            <div class="ld ld-ring ld-cycle"></div>
                        </button>`;
                    }
...
function get_default_excel_template(){
    $.ajax({
        type: "POST",
        url: "/webservice/insurance",
        headers:{
            'action': 'get_default_excel_template',
        },
        data: {
           'total_pax': total_pax,
        ...
        success: function(msg) {
            console.log(msg)
            if(msg.result.error_code == 0){
                saveAsXlsxFile(msg.result.response);
            }else{
                Swal.fire({
                  type: 'error',
                  title: 'Oops!',
...
function saveAsXlsxFile(data){
    ...
    //a.href = mediaType+userInp;
    a.download = 'template excel insurance.xlsx';
