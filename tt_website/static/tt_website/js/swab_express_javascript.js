...
function update_timeslot(val){
    ...
    change_timeslot(val);
    document.getElementById('next_swab_express').style.display='none';
    try{
        document.getElementById('swab_express_pax_div').hidden = true;
    }catch(err){
...
function get_area_global(){
    ...
    if(test_type.includes('PHCHC')){
        ...
        $('html, body').animate({
            scrollTop: $("#div_schedule_swab_express").offset().top - 110
        }, 500);
    }
}
...
function change_timeslot(val){
    ...
    document.getElementById('swab_express_detail').style.display = 'none';
}
...
function regex_phone_swab_express(value){
  var regex_check = "^[1-9][0-9]*$";//number
    if(value.match(regex_check)!=null){
        return value
    }else{
        value = regex_phone_swab_express(value.slice(1,value.length));
        return value;
    }
}
...
function onchange_number_no_zero(val){
    if(document.getElementById('adult_phone'+val).value != '' && document.getElementById('adult_phone'+val).value != undefined){
        var data = regex_phone_swab_express(document.getElementById('adult_phone'+val).value)
...
//                }
    ...
    //       re_medical_signin('next');
            ...
            document.getElementById('swab_express_review').submit();
       }else{
           ...
           if(check_scroll == "booker"){
                ...
                $('html, body').animate({
                    scrollTop: $("#swab_express_pax_div").offset().top - 120
                }, 500);
           }else{
                $('html, body').animate({
...
function add_table(change_rebooking=false){
    ...
    if(change_rebooking == true && total_passengers_rebooking != 0){
        ...
        // isi data passenger
        get_data_cache_passenger_swab_express();
        get_data_cache_schedule_swab_express();
...
function add_table_verify(change_rebooking=false){
    ...
    if(change_rebooking == true && total_passengers_rebooking != 0){
        ...
        last_counter = tempcounter;
        // isi data
        get_data_cache_passenger_swab_express('verify');
    }
    last_counter = tempcounter;
}
...
function next_prev_form_medical(type, counter, id){
    ...
    length_progress = $(".progress_swab_express"+pax).length;
