...
//                                        }
    ...
    //                            }
                    ...
                    if(msg.result.response.state == 'booked' && user_login.co_agent_frontend_security.includes('b2c_limitation') == false && user_login.co_agent_frontend_security.includes("corp_limitation") == false)
                     price_text+=`
                     <div style="text-align:right; padding-bottom:10px; margin-top:10px;"><img src="/static/tt_website/img/bank.png" alt="Bank" style="width:auto; height:25px; cursor:pointer;" onclick="show_repricing();"/></div>`;
                        ...
                        <div class="col-lg-12" style="padding-bottom:10px;">
                            ...
                            if (isMobile) {
                                price_text+=`
                                    ...
                                    <a href="mailto:?subject=This is the tour price detail&amp;body=`+ $text_share +`" title="Share by Email" style="padding-right:5px;" target="_blank"><img style="height:30px; width:auto;" src="/static/tt_website/img/email.png" alt="Email"/></a>`;
                            } else {
                                price_text+=`
                                ...
                                for(k in tr_get_booking.result.response.passengers[i].sale_service_charges[j]){
                                    if(k != 'RAC'){
                                        ...
                                        if(currency_breakdown == '')
                                            currency_breakdown = tr_get_booking.result.response.passengers[i].sale_service_charges[j][k].currency;
                                    }
                                }
                            }
...
function table_price_update(msg,type){
    ...
    if(document.URL.split('/')[document.URL.split('/').length-1] == 'review' && user_login.co_agent_frontend_security.includes('b2c_limitation') == false && user_login.co_agent_frontend_security.includes("corp_limitation") == false){
        price_txt +=`<div style="text-align:right;"><img src="/static/tt_website/img/bank.png" alt="Bank" style="width:auto; height:25px; cursor:pointer;" onclick="show_repricing();"/></div>`;
    }
    // pindah ke pax
//    try{
//        if(upsell_price != 0){
