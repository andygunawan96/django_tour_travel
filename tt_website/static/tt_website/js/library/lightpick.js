...
function next_focus_after_date(id){
    if(id != 'none'){
        setTimeout(function(){
            $(id).click();
        }, 200);
  ...
  var document = window.document,
    ...
    renderTopButtons = function(opts) {
      return (
        ...
        '' +
        '<button type="button" id="lightpick_previous" class="lightpick__previous-action">' +
        opts.locale.buttons.prev +
        '</button>' +
        '<button type="button" id="lightpick_next" class="lightpick__next-action">' +
    ...
    renderCalendar = function(el, opts) {
      ...
      var totalMonths = parseInt((moment(opts.maxDate).diff(moment(opts.minDate), 'months', true)))+1;

      if (moment(opts.maxDate).date() < moment(opts.minDate).date()){
      ...
      for (var i = 0; i < totalMonths; i++) {
        ...
        html +=
          '<div class="lightpick__month-title" style="text-align:center;">' +
           tempMonth+ ', ' +
           tempYear +
          '</div>';
...
//        }
        ...
        for (var w = opts.firstDay + 4; w < 7 + opts.firstDay + 4; ++w) {
          //edited
          var day_check = weekdayName(opts, w, 'long');
          if(day_check == 'Sunday'){
            html += `<div class="lightpick__day-of-the-week lightpick__day-of-the-week-sun"`;
          }else{
            html += `<div class="lightpick__day-of-the-week"`;
          }
          //edited
...
//        (opts.numberOfMonths > 1 ? renderTopButtons(opts, 'days') : '') + //edit
          ...
          if (
            ...
            if (opts.repick && opts.startDate && opts.endDate) {
              ...
              if (opts.singleDate && opts.autoclose) {
                setTimeout(function() {
                  next_focus_after_date(opts.nextFocus);
                  self.hide();
                }, 100);
      ...
      self.el.addEventListener('click', self._onMouseClick, true);
    ...
    updatePosition: function() {
      ...
      if(this._opts.parentEl == 'body'){
          this.el.style.top = top + 'px';
          if(left <= 0){
            this.el.style.left = 5 + 'px';
          }else{
            this.el.style.left = left + 'px';
          }
      }else{
          this.el.style.top = parseInt(top-50) + 'px';
...
//        }
    ...
    show: function(target) {
      if (!this.isShowing) {
        ...
        this.gotoDate(this._opts.minDate);
    ...
    destroy: function() {
      ...
      this.el.removeEventListener('click', self._onMouseClick, true);
