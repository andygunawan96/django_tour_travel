function sentra_medika_signin(data){
    getToken();
    $.ajax({
       type: "POST",
...
//       url: "{% url 'tt_backend_rodextrip:social_media_tree_update' %}",
       ...
       try{
           if(msg.result.error_code == 0){
               ...
               if(data == 'passenger'){
                    get_config_sentra_medika(data, vendor);
                    sentra_medika_get_availability();
               }else{
                    //get booking
                    get_config_sentra_medika('get_booking');
       ...
       error: function(XMLHttpRequest, textStatus, errorThrown) {
          error_ajax(XMLHttpRequest, textStatus, errorThrown, 'Error sentra medika signin');
...
function sentra_medika_page_passenger(){
    $.ajax({
       type: "POST",
       url: "/webservice/sentra_medika",
       headers:{
...
function sentra_medika_page_review(){
    $.ajax({
       type: "POST",
       url: "/webservice/sentra_medika",
       headers:{
...
function get_config_sentra_medika(type){
    $.ajax({
       type: "POST",
       url: "/webservice/sentra_medika",
       headers:{
       ...
       success: function(msg) {
            if(msg.result.error_code == 0){
                ...
                if(type == 'home'){
                    ...
                    document.getElementById('medical_type_sentra_medika').innerHTML += text;
                    $('#medical_type_sentra_medika').niceSelect('update');
                }else if(type == 'passenger'){
                    print_check_price++;
                    if(print_check_price == 2){
                        document.getElementById('check_price_sentra_medika').hidden = false;
                    ...
                    document.getElementById('sentra_medika_product').innerHTML = product;
...
function sentra_medika_get_availability(){
    test_date_data = [];
    test_kuota = [];
    $.ajax({
       type: "POST",
       ...
       error: function(XMLHttpRequest, textStatus, errorThrown) {
            error_ajax(XMLHttpRequest, textStatus, errorThrown, 'Error get availability sentra_medika');
       },timeout: 300000
    });
}

function sentra_medika_check_price(){
    var timeslot_list = [];
    document.getElementById('check_price_sentra_medika').disabled = true;
        ...
        $.ajax({
           ...
           success: function(msg) {
                ...
                if(msg.result.error_code == 0){
                    ...
                    document.getElementById('next_sentra_medika').style.display = 'block';

                    try{
                        document.getElementById('sentra_medika_pax_div').hidden = false;
                    }catch(err){
                        console.log(err); // error kalau ada element yg tidak ada
                    }
                    $('html, body').animate({
                        scrollTop: $("#sentra_medika_detail").offset().top - 120
                    }, 500);
                ...
                document.getElementById('check_price_sentra_medika').disabled = false;
           },
           error: function(XMLHttpRequest, textStatus, errorThrown) {
                error_ajax(XMLHttpRequest, textStatus, errorThrown, 'Error get price sentra_medika');
                document.getElementById('check_price_sentra_medika').disabled = false; //disable false jika timeout atau apapun yg masuk catch
           },timeout: 300000
        });
    }else if(error_log != ''){
        Swal.fire({
...
function sentra_medika_get_cache_price(){
    $.ajax({
       type: "POST",
       url: "/webservice/sentra_medika",
       headers:{
...
function pre_sentra_medika_commit_booking(val){
    if(val == 0){
        sentra_medika_commit_booking(val);
       ...
       success: function(msg) {
           if(msg.result.error_code == 0){
                ...
                Swal.fire({
                  type: 'error',
                  title: 'Oops!',
                  html: '<span style="color: #ff9900;">Error sentra_medika confirm order </span>' + msg.result.error_msg,
                })
           }
...
function sentra_medika_commit_booking(val){
    $('.hold-seat-booking-train').addClass("running");
    $('.hold-seat-booking-train').attr("disabled", true);
    please_wait_transaction();
...
//                            }else if(val == 1){
//                                document.getElementById('order_number').value = msg.result.response.order_number;
//                                document.getElementById('issued').action = '/sentra_medika/booking/' + btoa(msg.result.response.order_number);
...
//        document.getElementById('sentra_medika_edit_passenger').action += medical_get_detail.result.response.order_number;
//        document.getElementById('sentra_medika_edit_passenger').submit();
//    }
//}

function sentra_medika_get_booking(order_number, sync=false){
    document.getElementById('payment_acq').hidden = true;
    price_arr_repricing = {};
    ...
    $.ajax({
       ...
       success: function(msg) {
            try{
                ...
                //======================= Resv =========================
                if(msg.result.error_code == 0){
                    if(window.location.pathname.includes('confirm_order') && user_login.co_agent_frontend_security.includes('confirm_order_sentra_medika') == false){
                        window.location.href = '/sentra_medika/confirm_order/';
                        Swal.fire({
                          type: 'error',
                          title: 'Oops!',
    ...
    //                    document.getElementById('new-reservation').hidden = false;
                        ...
                        if(msg.result.response.state == 'booked'){
                           check_payment_payment_method(msg.result.response.order_number, 'Issued', msg.result.response.booker.seq_id, 'billing', 'sentramedika', signature, msg.result.response.payment_acquirer_number);
                           ...
                           <div class="alert alert-success" role="alert">
                            ...
                            document.getElementById('show_title_sentra_medika').hidden = true;
                       ...
                       text+=`</div>
                        ...
                        document.getElementById('sentra_medika_booking').innerHTML = text;
    ...
    //                            }
                        ...
                        try{
                            ...
                            if(msg.result.response.state == 'booked' && user_login.co_agent_frontend_security.includes('b2c_limitation') == false && user_login.co_agent_frontend_security.includes("corp_limitation") == false)
                                text_detail+=`<div style="text-align:right; padding-bottom:10px;"><img src="/static/tt_website/img/bank.png" alt="Bank" style="width:auto; height:25px; cursor:pointer;" onclick="show_repricing();"/></div>`;
                                ...
                                text_detail+=`<div style="text-align:right; padding-bottom:10px;"><img src="/static/tt_website/img/banknew.png" alt="Bank" style="width:auto; height:25px; cursor:pointer;" onclick="show_repricing();"/></div>`;
                            ...
                            <div class="col-lg-12" style="padding-bottom:10px;">
                                ...
                                if (isMobile) {
                                    text_detail+=`
                                        ...
                                        <a href="mailto:?subject=This is the airline price detail&amp;body=`+ $text_share +`" title="Share by Email" style="padding-right:5px;" target="_blank"><img style="height:30px; width:auto;" src="/static/tt_website/img/email.png" alt="Email"/></a>`;
                                } else {
                                    text_detail+=`
        ...
        //                    </div>`;
                            ...
                            if(window.location.pathname.includes('confirm_order') && user_login.co_agent_frontend_security.includes('confirm_order_sentra_medika') && msg.result.response.state_vendor == 'new_order' && moment().format('YYYY-MM-DD') == msg.result.response.picked_timeslot.datetimeslot.substr(0,10)){
                                text_detail+=`
        ...
        //                        change_area();
                        ...
                        document.getElementById('sentra_medika_detail').innerHTML = text_detail;
                        ...
                        if(window.location.pathname.includes('confirm_order') == false){
                            ...
                            if (msg.result.response.state  == 'issued') {
                                print_text+=`
                                <button class="primary-btn-white hold-seat-booking-train ld-ext-right" id="button-choose-print" type="button" onclick="get_printout('` + msg.result.response.order_number + `','ticket','sentramedika');" style="width:100%;">
                                    Print Ticket
                                    <div class="ld ld-ring ld-cycle"></div>
                                </button>`;
                            }*/
                            ...
                            if (msg.result.response.state  == 'issued'){
                                print_text+=`
                                <button class="primary-btn-white hold-seat-booking-train ld-ext-right" type="button" id="button-print-print" onclick="get_printout('` + msg.result.response.order_number + `','ticket_price','sentramedika');" style="width:100%;">
                                                ...
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-body">
                                                        ...
                                                        <div style="text-align:right;">
                                                            ...
                                                            <br/>
                                                            <button type="button" id="button-issued-print" class="primary-btn ld-ext-right" onclick="get_printout('`+msg.result.response.order_number+`', 'invoice','sentramedika');">
                                                                Submit
                                                                <div class="ld ld-ring ld-cycle"></div>
                                                            </button>
                                                        </div>
                                                ...
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-body">
                                                        ...
                                                        <div style="text-align:right;">
                                                            <span>Don't want to edit? just submit</span>
                                                            <br/>
                                                            <button type="button" id="button-issued-print" class="primary-btn ld-ext-right" onclick="get_printout('`+msg.result.response.order_number+`', 'kwitansi','sentra_medika');">
                                                                Submit
                                                                <div class="ld ld-ring ld-cycle"></div>
                                                            </button>
                                                        </div>
                        ...
                        if(print_text)
                            document.getElementById('sentra_medika_btn_printout').innerHTML = print_text;
                    ...
                    document.getElementById('show_title_sentra_medika').hidden = false;
                    document.getElementById('show_loading_booking_sentra_medika').hidden = true;
                    document.getElementById('show_title_sentra_medika').hidden = true;
                ...
                Swal.fire({
                  type: 'error',
                  title: 'Oops!',
                  html: '<span style="color: #ff9900;">Error sentra_medika booking </span> Please try again in 1 - 5 minutes later or contact customer service' ,
                }).then((result) => {
...
function sentra_medika_cancel_booking(data){
    var temp_data = {}
...
//                        document.getElementById('button-print-print').hidden = true;
                    ...
                    Swal.fire({
                      type: 'error',
                      title: 'Oops!',
                      html: '<span style="color: #ff9900;">Error sentra_medika cancel </span>' + msg.result.error_msg,
                    })
                    price_arr_repricing = {};
                    pax_type_repricing = [];
                    document.getElementById('show_loading_booking_sentra_medika').hidden = false;
...
function sentra_medika_issued_booking(data){
    var temp_data = {}
    if(typeof(medical_get_detail) !== 'undefined')
        temp_data = JSON.stringify(medical_get_detail)
    Swal.fire({
...
function sentra_medika_get_result(data){
    var temp_data = {}
    if(typeof(medical_get_detail) !== 'undefined')
        temp_data = JSON.stringify(medical_get_detail)
    show_loading();
...
function create_new_reservation(){
    ...
    //button
    text += `<button type="button" class="primary-btn mb-3" id="button-home" style="width:100%;margin-top:15px;" onclick="sentra_medika_reorder();">
                Re Order
            </button>`
...
function sentra_medika_reorder(){
    //check all pax
    var checked = false;
    ...
    if(checked){
        var path = '/sentra_medika/passenger/' + document.getElementById('test_type').value;
        ...
        document.getElementById('sentra_medika_edit_passenger').action = path;
        document.getElementById('sentra_medika_edit_passenger').submit();
    }else{
        Swal.fire({
            type: 'error',
...
function get_data_cache_passenger_sentra_medika(type){
    $.ajax({
       type: "POST",
       url: "/webservice/sentra_medika",
       headers:{
            'action': 'get_data_cache_passenger_sentra_medika',
       },
       data: {
            'signature': signature,
       },
...
function get_data_cache_schedule_sentra_medika(){
    $.ajax({
       type: "POST",
       url: "/webservice/sentra_medika",
       headers:{
            'action': 'get_data_booking_cache_sentra_medika',
       },
       data: {
            'signature': signature,
       },
       ...
       error: function(XMLHttpRequest, textStatus, errorThrown) {
            error_ajax(XMLHttpRequest, textStatus, errorThrown, 'Error get cache data schedule sentra medika');
       },timeout: 300000
    });
       ...
       success: function(msg) {
           if(msg.result.error_code == 0){
                if(type == 'booking'){
                    ...
                    pax_type_repricing = [];
                    please_wait_transaction();
                    sentra_medika_get_booking(repricing_order_number);
                }else{
                    price_arr_repricing = {};
                    pax_type_repricing = [];
                    train_detail();
                ...
                Swal.fire({
                  type: 'error',
                  title: 'Oops!',
                  html: '<span style="color: #ff9900;">Error sentra medika service charge </span>' + msg.result.error_msg,
                })
           }
       },
       error: function(XMLHttpRequest, textStatus, errorThrown) {
            error_ajax(XMLHttpRequest, textStatus, errorThrown, 'Error sentra medika service charge');
       },timeout: 480000
    });
       ...
       success: function(msg) {
           if(msg.result.error_code == 0){
                ...
                Swal.fire({
                  type: 'error',
                  title: 'Oops!',
                  html: '<span style="color: #ff9900;">Error sentra medika update booker insentif </span>' + msg.result.error_msg,
                })
                $('.loader-rodextrip').fadeOut();
           }
       },
       error: function(XMLHttpRequest, textStatus, errorThrown) {
            error_ajax(XMLHttpRequest, textStatus, errorThrown, 'Error sentra medika update booker insentif');
            $('.loader-rodextrip').fadeOut();
       },timeout: 60000
    });
