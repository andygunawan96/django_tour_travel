...
function add_promotion_code(car_code='', osi_code=''){
    text = '';
    if(car_code != '' && osi_code != ''){
        text += `
        <div class="row">
            <div class="col-lg-12" style="margin:auto;">
                <div style="display:inline-flex;">
                    <div style="display:inline-block; margin:auto; padding-right:5px;">
                        <i class="fas fa-ticket-alt" style="font-size:18px;"></i>
                    </div>
                    <div style="display:inline-block;padding-left:5px;border-left:1px solid #cdcdcd;">
                        <h6>Promo<br/>`+osi_code+` `+car_code+`</h6>
                    </div>
                </div>
        ...
        node.className = 'div_for_add';
        node.id = 'promotion_code_line' + promotion_code;
        node.innerHTML = text;
        document.getElementById("promotion_code").appendChild(node);
        promotion_code++;
        total_promo_code_chosen++;
        document.getElementById('show_total_promo_code').innerHTML = total_promo_code_chosen + ' promo codes';
        document.getElementById('span_promo_code').innerHTML = '';
        document.getElementById('promo_code_counter').value = promotion_code;
        document.getElementById('code_line_input').value = '';
        document.getElementById('carrier_code_line_input').value = '';
    }
    else{
        text_code = document.getElementById('code_line_input').value;
        text_carrier_code = document.getElementById('carrier_code_line_input').value;

        if(text_code != '' && text_carrier_code != ''){
            text += `
            <div class="row">
                <div class="col-lg-12" style="margin:auto;">
                    <div style="display:inline-flex;">
                        ...
                        <div style="display:inline-block;padding-left:5px;border-left:1px solid #cdcdcd;">
                            <h6>Promo<br/>`+text_code+` `+text_carrier_code+`</h6>
                        </div>
                    </div>
                    <i class="fa fa-trash" onclick="delete_promotion_code(`+promotion_code+`)" style="color: #ff0002; font-size:18px; position: absolute; right: 15px; top: 5px;"></i>
                </div>
...
function toggle_promo_code(){
    if(document.getElementById('checkbox_add_promotion_code_airline').checked){
        document.getElementById('promo_code_airline').style.display = 'inline-flex';
    }else{
...
function delete_promotion_code(val){
    ...
    total_promo_code_chosen--;
    if(total_promo_code_chosen != 0){
        document.getElementById('show_total_promo_code').innerHTML = total_promo_code_chosen + ' promo codes';
    }else{
        document.getElementById('show_total_promo_code').innerHTML = 'Add promo code';
        document.getElementById('span_promo_code').innerHTML = 'No Promo Codes Added';
    }
}
...
//    error_log = ''; //DEV GARUDA
    if(error_log == ''){
        ...
        if(direction == 'oneway')
            ...
            direction = 'MC'
        if(['OW', 'RT'].includes(direction)){
            request_airline = {
                "origin": [document.getElementById('origin_id_flight').value],
                "destination": [document.getElementById('destination_id_flight').value],
                "departure": [direction == 'RT' ? document.getElementById('airline_departure_return').value.split(' - ')[0] : document.getElementById('airline_departure').value],
                "return": [direction == 'RT' ? document.getElementById('airline_departure_return').value.split(' - ')[1] : ''],
                "direction": direction,
            ...
            if(direction == 'RT'){
                request_airline['origin'].push(document.getElementById('destination_id_flight').value)
                request_airline['destination'].push(document.getElementById('origin_id_flight').value)
                request_airline['departure'].push(document.getElementById('airline_departure_return').value.split(' - ')[1])
            }
    ...
    if(counter_airline_search != 4){
        ...
        if(counter_airline_search == 1){
            ...
            <div class="div-dropdown-txt btn-group" style="margin-right:0px;">
                <div class="input-container-search-ticket btn-group" style="padding-top:10px;">
                    <span class="span-search-ticket" style="padding-right:5px; margin-top:-1px; font-size:14px;">Preferred Class </span>
                    <div class="form-select form-select-text class_airline_select" id="default-select" style="height:15px; width:fit-content; margin-bottom: 0px;">
                        <select id="cabin_class_flight_mc" name="cabin_class_flight_mc" class="nice-select-text" onchange="next_focus_element('airline', 'preferred');">`;
        ...
        if(type == 'search'){
            ...
            picker_multi_city[counter_airline_search] = new Lightpick({
                ...
                onSelect: function(date){
                    ...
                    next_focus_element('airline', '.class_airline_select'+this._opts.idNumber);
                }
            });
...
function airline_filter_render(){
    ...
    var node = document.createElement("div");
    text = '';
    text+= `<h5 style="display: inline;">Filter</h5><a style="float: right; cursor: pointer;" onclick="reset_filter();"><i style="color:`+color+`;" class="fa fa-refresh"></i> Reset</a>
    <hr/>
...
//                                            price_breakdown['COMMISSION'] = (airline_recommendations_journey[airline_recommendations_list.indexOf(airline[i].journey_ref_id)].service_charge_summary[l].base_commission_vendor * -1);
//                                            price_breakdown['NTA AIRLINE'] = airline_recommendations_journey[airline_recommendations_list.indexOf(airline[i].journey_ref_id)].service_charge_summary[l].base_nta_vendor;
...
//                                                            price_breakdown['BREAKDOWN'] = 0;
//                                                            price_breakdown['COMMISSION'] -= (airline_pick_list[index_airline].segments[index_segment].fares[index_fare].service_charge_summary[index_svc_summary].base_commission_vendor * -1);
//                                                            price_breakdown['NTA AIRLINE'] -= airline_pick_list[index_airline].segments[index_segment].fares[index_fare].service_charge_summary[index_svc_summary].base_nta_vendor;
...
function airline_pick_mc(type){
    ...
    for(i in airline_pick_list){
        text+=`
        <div style="background-color:white; box-shadow: rgb(3 18 26 / 15%) 4px -1px 10px -4px, rgb(3 18 26 / 15%) -4px -1px 10px -4px; border-radius:5px; border:1px solid #cdcdcd; margin-bottom:15px; padding:15px 15px 0px 15px;" id="journey_pick`+airline_pick_list[i].airline_pick_sequence+`">
            <div class="row">
                <div class="col-lg-12 mb-3" style="border-bottom:1px solid #cdcdcd; padding-bottom:10px;">`;
                    text+=`
                    <span style="font-size:14px; font-weight:bold;"><img style="width:auto; height:25px; border-radius:7px; background:white;" src="/static/tt_website/images/icon/product/c-airline.png" alt="Airline Icon"> `;
                    if($radio_value_string != "multicity"){
                        ...
                        text+=`Flight #`+(airline_pick_list[i].airline_pick_sequence)+``;
                    }
                    text+=` |
                    </span>
                    <span> `+airline_request.origin[i].split(' - ')[1] + ` (`+airline_request.origin[i].split(' - ')[0]+`) <i class="fas fa-arrow-right"></i> `+airline_request.destination[i].split(' - ')[1]+` (`+airline_request.destination[i].split(' - ')[0]+`) | `+airline_request.departure[i]+`</span>`;
                text+=`
                </div>
                <div class="col-lg-12">`;
                    carrier_code_airline = [];
                       ...
                       for(banner_counter in airline_pick_list[i].search_banner){
                            ...
                            if(selected_banner_date >= max_banner_date){
                                if(airline_pick_list[i].search_banner[banner_counter].active == true){
                                    text+=`<label id="pop_search_banner_pick`+i+``+banner_counter+`" style="background:`+airline_pick_list[i].search_banner[banner_counter].banner_color+`; color:`+airline_pick_list[i].search_banner[banner_counter].text_color+`;padding:5px 10px; margin-right:5px;">`+airline_pick_list[i].search_banner[banner_counter].name+`</label>`;
                                }
                            }
                        }
                    }
    ...
    //                    }
                                        ...
                                        for(k in currency_rate_data.result.response.agent[user_login.co_ho_seq_id]){
                                            try{
                                                if(currency_rate_data.result.response.agent[user_login.co_ho_seq_id][k].base_currency == currency){
                                                    price_convert = ((price-total_discount)/currency_rate_data.result.response.agent[user_login.co_ho_seq_id][k].rate).toFixed(2);
                                                    if(price_convert%1 == 0)
                                                        price_convert = parseInt(price_convert);
                        ...
                        <div class="col-lg-12 col-md-6 pt-3" style="margin:auto;">
                            ...
                            if(type == 'all'){
                                ...
                                <div class="col-lg-12 col-md-6">
                                    <input type='button' style="width:100%; height:35px; line-height:unset;" id="changejourney_pick`+airline_pick_list[i].airline_pick_sequence+`" class="primary-btn-gr choose_selection_ticket_airlines_depart" value="Change" onclick="change_departure(`+airline_pick_list[i].airline_pick_sequence+`); change_search_to_api(`+parseInt(airline_pick_list[i].airline_pick_sequence-1)+`);" sequence_id="0"/>
                                </div>`;
                            }
                            else if(type == 'change'){
                                text+=`
                                ...
                                <div class="col-lg-12 col-md-6">
                                    <input type='button' style="width:100%; height:35px; line-height:unset; background:#f5f5f5 !important;" id="changejourney_pick`+airline_pick_list[i].airline_pick_sequence+`" class="primary-btn-gr choose_selection_ticket_airlines_depart" value="Change" onclick="change_departure(`+airline_pick_list[i].airline_pick_sequence+`);" disabled sequence_id="0"/>
                                </div>`;
                            }
                            ...
                            <div class="row" id="journeypick0segment0">
                               <div class="col-lg-3 mb-2">
                                   ...
                                   try{
                                       ...
                                       text+=`
                                       <div style="display:inline-block;">
                                            <img data-toggle="tooltip" class="airlines_provider_img_fd" alt="/static/tt_website/images/icon/product/c-airline.png" title="Airline" src="/static/tt_website/images/icon/product/c-airline.png">
                                       </div>
                                       <div style="display:inline-block; margin:auto;">
                                           <div style="display:grid;">
...
//                                </div>`;
                                               ...
                                               <div style="display:flex; overflow:auto;">`;
                                                   for(k in airline_pick_list[i].segments[j].fares){
                                                        ...
                                                        if(k==airline_pick_list[i].segments[j].fare_pick){
                                                            ...
                                                            text+=`
                                                            <label class="radio-label100">
                                                                <input id="journeypick`+airline_pick_list[i].airline_pick_sequence+`segment`+j+`fare" name="journeypick`+airline_pick_list[i].airline_pick_sequence+`segment`+j+`fare" type="radio" value="`+k+`" onclick="change_fare_airline_pick_list(`+i+`,`+j+`,`+k+`);">
                                                                <div class="div_label100">`;
    ...
    for(i in airline_pick_list){
       ...
       show_flight_details2(airline_pick_list[i].airline_pick_sequence);
    }
    new jBox('Tooltip', {
...
//                        price_breakdown['BREAKDOWN'] = 0;
//                        price_breakdown['COMMISSION'] += (airline_pick_list[i].segments[j].fares[airline_pick_list[i].segments[j].fare_pick].service_charge_summary[k].base_commission_vendor * -1);
...
//                        price_breakdown['NTA AGENT'] += airline_pick_list[i].segments[j].fares[airline_pick_list[i].segments[j].fare_pick].service_charge_summary[k].base_nta;
//                        if(user_login.co_agent_frontend_security.includes('agent_ho'))
//                            price_breakdown['COMMISSION HO'] += airline_pick_list[i].segments[j].fares[airline_pick_list[i].segments[j].fare_pick].service_charge_summary[k].base_commission_ho * -1;
                        break;
                    }
                }
            }
...
function change_fare_airline_pick_list(journey_key, segment_key, fare_key){
    // check airline
//    var is_need_unchoose_journey = false;
//    for(x in airline_pick_list){
//        if(is_need_unchoose_journey){
//            document.getElementById("journey_pick"+airline_pick_list[x].airline_pick_sequence).remove();
//        }
//        if(x == journey_key){
//            if(airline_pick_list.length-1 != x)
//                is_need_unchoose_journey = true;
//        }
//    }
...
function first_value_provider(){
    ...
    if(document.getElementById('provider_box_All').checked == true){
        try{
            document.getElementById('show_provider_airline').innerHTML = 'All airline';
        }catch(err){
            console.log(err)
        }
        try{
        ...
        try{
            ...
            if(val == undefined && carrier_code == undefined){
                if(check == 0){
                    ...
                    document.getElementById('show_provider_airline').innerHTML = 'All airline';
                    try{
                        document.getElementById('show_provider_airline1').innerHTML = 'All airline';
                        document.getElementById('provider_box_All_1').checked = true
                    }catch(err){console.log(err);}
                }else{
                    document.getElementById('show_provider_airline').innerHTML = check+' Airline chosen';
...
//                                </span>
                    ...
                    //                    try{
                    //                        text_modal_content+=`<img data-toggle="tooltip" alt="`+airline_carriers[0][carrier_code_list[k]]+`" title="`+airline_carriers[0][carrier_code_list[k]]+`" style="margin-top:10px; width:50px; height:50px; margin-right:10px;" src="`+static_path_url_server+`/public/airline_logo/`+carrier_code_list[k]+`.png"><span> </span>`;
                    //                    }catch(err){
                    //                        text_modal_content+=`<img data-toggle="tooltip" alt="Airline" title="" style="margin-top:10px; width:50px; height:50px; margin-right:10px;" src="`+static_path_url_server+`/public/airline_logo/`+carrier_code_list[k]+`.png"><span> </span>`;
                    //                    }
                                    ...
                                    <div class="row">`;
                                        for(k in price_itinerary_temp[i].journeys[j].segments){
                                            ...
                                            <div class="col-lg-4 col-md-4">
                                                <div style="display:inline-flex;">
                                                    <div style="display:inline-block;">`;
                                                        try{
                                                            if(price_itinerary_temp[i].journeys[j].segments[k].carrier_code != price_itinerary_temp[i].journeys[j].segments[k].operating_airline_code && price_itinerary_temp[i].journeys[j].segments[k].operating_airline_code != ''){
                                                                text_modal_content+=`<img data-toggle="tooltip" class="airlines_provider_img" alt="`+airline_carriers[price_itinerary_temp[i].journeys[j].segments[k].operating_airline_code].name+`" title="`+airline_carriers[price_itinerary_temp[i].journeys[j].segments[k].operating_airline_code].name+`" src="`+static_path_url_server+`/public/airline_logo/`+price_itinerary_temp[i].journeys[j].segments[k].operating_airline_code+`.png">`;
                                                            }else{
                                                                text_modal_content+=`<img data-toggle="tooltip" class="airlines_provider_img" alt="`+airline_carriers[price_itinerary_temp[i].journeys[j].segments[k].carrier_code].name+`" title="`+airline_carriers[price_itinerary_temp[i].journeys[j].segments[k].carrier_code].name+`" src="`+static_path_url_server+`/public/airline_logo/`+price_itinerary_temp[i].journeys[j].segments[k].carrier_code+`.png">`;
                                                            }
                                                        }catch(err){
                                                            // carrier tidak ketemu di dict
                                                            if(price_itinerary_temp[i].journeys[j].segments[k].carrier_code != price_itinerary_temp[i].journeys[j].segments[k].operating_airline_code && price_itinerary_temp[i].journeys[j].segments[k].operating_airline_code != ''){
                                                                text_modal_content+=`<img data-toggle="tooltip" class="airlines_provider_img" alt="`+price_itinerary_temp[i].journeys[j].segments[k].operating_airline_code+`" title="`+price_itinerary_temp[i].journeys[j].segments[k].operating_airline_code+`" src="`+static_path_url_server+`/public/airline_logo/`+price_itinerary_temp[i].journeys[j].segments[k].operating_airline_code+`.png">`;
                                                            }else{
                                                                text_modal_content+=`<img data-toggle="tooltip" class="airlines_provider_img" alt="`+price_itinerary_temp[i].journeys[j].segments[k].carrier_code+`" title="`+price_itinerary_temp[i].journeys[j].segments[k].carrier_code+`" src="`+static_path_url_server+`/public/airline_logo/`+price_itinerary_temp[i].journeys[j].segments[k].carrier_code+`.png">`;
                                                            }
                                                        }
                                                    text_modal_content+=`
                                                    </div>
                                                    <div style="display:inline-block;">`;
                                                        try{
                                                            if(price_itinerary_temp[i].journeys[j].segments[k].carrier_code != price_itinerary_temp[i].journeys[j].segments[k].operating_airline_code && price_itinerary_temp[i].journeys[j].segments[k].operating_airline_code != ''){
                                                                text_modal_content+=`<span style="margin-top:15px; font-size:13px; font-weight:500;">Operated By `+airline_carriers[price_itinerary_temp[i].journeys[j].segments[k].operating_airline_code].name+` (`+price_itinerary_temp[i].journeys[j].segments[k].carrier_code+price_itinerary_temp[i].journeys[j].segments[k].carrier_number+`)</span><br/>`;
                                                            }else{
                                                                text_modal_content+=`<span style="margin-top:15px; font-size:13px; font-weight:500;">`+airline_carriers[price_itinerary_temp[i].journeys[j].segments[k].carrier_code].name+` (`+price_itinerary_temp[i].journeys[j].segments[k].carrier_code+price_itinerary_temp[i].journeys[j].segments[k].carrier_number+`)</span><br/>`;
                                                            }
                                                            ...
                                                            // carrier tidak ketemu di dict
                                                            if(price_itinerary_temp[i].journeys[j].segments[k].carrier_code != price_itinerary_temp[i].journeys[j].segments[k].operating_airline_code && price_itinerary_temp[i].journeys[j].segments[k].operating_airline_code != ''){
                                                                text_modal_content+=`<span style="margin-top:15px; font-size:13px; font-weight:500;">Operated By`+price_itinerary_temp[i].journeys[j].segments[k].operating_airline_code+` (`+price_itinerary_temp[i].journeys[j].segments[k].carrier_code+price_itinerary_temp[i].journeys[j].segments[k].carrier_number+`)</span><br/>`;
                                                            }else{
                                                                text_modal_content+=`<span style="margin-top:15px; font-size:13px; font-weight:500;">`+price_itinerary_temp[i].journeys[j].segments[k].carrier_code+` (`+price_itinerary_temp[i].journeys[j].segments[k].carrier_code+price_itinerary_temp[i].journeys[j].segments[k].carrier_number+`)</span><br/>`;
                                                            }
                                            ...
                                            <div class="col-lg-8 col-md-8">
                                                <div class="row">
                                                    <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">
                                                        <h6>`+price_itinerary_temp[i].journeys[j].segments[k].departure_date.split(' - ')[1]+`</h6>
                                                        <span>`+price_itinerary_temp[i].journeys[j].segments[k].departure_date.split(' - ')[0]+` </span><br/>
                                                        <span style="font-weight:500;">`+price_itinerary_temp[i].journeys[j].segments[k].origin_city+` (`+price_itinerary_temp[i].journeys[j].segments[k].origin+`)</span><br/>`;
                                                        if(price_itinerary_temp[i].journeys[j].segments[k].origin_terminal != ''){
                                                            text_modal_content+=`<span style="font-size:13px; font-weight:500;">Terminal: `+price_itinerary_temp[i].journeys[j].segments[k].origin_terminal+`</span><br/>`;
                                                        }else{
                                                            text_modal_content+=`<span style="font-size:13px; font-weight:500;">Terminal: -</span><br/>`;
                                                        }
                                                    text_modal_content+=`
                                                    </div>
                                                    <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">
                                                        <div style="text-align:center; position: absolute; left:-30%;">
                                                            <div style="display:inline-block;position:relative;width:150px;z-index:1;">
                                                                <img src="/static/tt_website/images/icon/symbol/airlines-01.png" alt="Airline" style="width:20px; height:20px; margin-top:5px; position:relative; z-index:99;"/>
                                                                <div class="show_pc" style="height:2px;position:absolute;top:16px;width:100%;background-color:#d4d4d4;"></div>
                                                                <div class="show_pc origin-code-snippet" style="background-color:#d4d4d4;right:0px"></div>
                                                            </div>
                                                        ...
                                                        <div style="text-align:right">
                                                            <h6>`+price_itinerary_temp[i].journeys[j].segments[k].arrival_date.split(' - ')[1]+`</h6>
                                                            <span>`+price_itinerary_temp[i].journeys[j].segments[k].arrival_date.split(' - ')[0]+`</span><br/>
                                                            <span style="font-weight:500;">`+price_itinerary_temp[i].journeys[j].segments[k].destination_city+` (`+price_itinerary_temp[i].journeys[j].segments[k].destination+`)</span><br/>`;
                                                            if(price_itinerary_temp[i].journeys[j].segments[k].destination_terminal != ''){
                                                                text_modal_content+=`<span style="font-size:13px; font-weight:500;">Terminal: `+price_itinerary_temp[i].journeys[j].segments[k].destination_terminal+`</span><br/>`;
                                                            }else{
                                                                text_modal_content+=`<span style="font-size:13px; font-weight:500;">Terminal: - </span><br/>`;
                                                            }
                                                        text_modal_content+=`
                                            ...
                                            <div class="col-lg-12">`;
                                                if(price_itinerary_temp[i].journeys[j].segments[k].fares.length > 0){
                                                    text_modal_content += `<div><img src="/static/tt_website/images/icon/symbol/seat.png" style="height:16px; width:auto;"><span style="font-weight:800;">`;
                                                    if(price_itinerary_temp[i].journeys[j].segments[k].fares[0].cabin_class != '')
                                                        if(price_itinerary_temp[i].journeys[j].segments[k].fares[0].cabin_class == 'Y')
                                                            ...
                                                            text_modal_content += ' (Royal Green)';
                                                        else if(price_itinerary_temp[i].journeys[j].segments[k].fares[0].cabin_class == 'W')
                                                            text_modal_content += 'Premium Economy - ';
                                                        else if(price_itinerary_temp[i].journeys[j].segments[k].fares[0].cabin_class == 'C')
                                                            text_modal_content += 'Business - ';
                                                        else if(price_itinerary_temp[i].journeys[j].segments[k].fares[0].cabin_class == 'F')
                                                            text_modal_content += 'First Class - ';
                                                    text_modal_content+=`Class: ` + price_itinerary_temp[i].journeys[j].segments[k].fares[0].class_of_service;
                                                    text_modal_content+=`</span>`;
                                                    for(l in price_itinerary_temp[i].journeys[j].segments[k].fares){
                                                        for(m in price_itinerary_temp[i].journeys[j].segments[k].fares[l].fare_details){
                                                            if(price_itinerary_temp[i].journeys[j].segments[k].fares[l].fare_details[m].detail_type.includes('BG')){
                    //                                            $text += '• Baggage ';
                    ...
                    //                                            $text += '\n';
                                                                text_modal_content += `<br/><i class="fas fa-suitcase"></i><span style="font-weight:800;"> Baggage - `+price_itinerary_temp[i].journeys[j].segments[k].fares[l].fare_details[m].amount + ' ' + price_itinerary_temp[i].journeys[j].segments[k].fares[l].fare_details[m].unit+` </span>`;
                                                            }else if(price_itinerary_temp[i].journeys[j].segments[k].fares[l].fare_details[m].detail_type.includes('ML')){
                    //                                            $text += '• Meal ' + price_itinerary_temp[i].journeys[j].segments[k].fares[l].fare_details[m].amount + ' ' + resJson.result.response.price_itinerary_provider[i].journeys[j].segments[k].fares[l].fare_details[m].unit + '\n';
                                                                text_modal_content += `<br/><i class="fas fa-suitcase"></i><span style="font-weight:800;"> Meal - `+price_itinerary_temp[i].journeys[j].segments[k].fares[l].fare_details[m].amount + ' ' + price_itinerary_temp[i].journeys[j].segments[k].fares[l].fare_details[m].unit+` </span>`;
                                                            }
                    ...
                    //                                        $text += '• Aircraft: ' + price_itinerary_temp[i].journeys[j].segments[k].carrier_type_name + '\n';
                                                            text_modal_content += `<br/><i class="fas fa-plane"></i><span style="font-weight:800;"> Aircraft - `+price_itinerary_temp[i].journeys[j].segments[k].carrier_type_name+` </span>`;
                                                        }
                    //                                    if(price_itinerary_temp[i].journeys[j].segments[k].fares[l].description.length > 0){
                    //                                        $text += 'Description: \n';
                    //                                        for(m in price_itinerary_temp[i].journeys[j].segments[k].fares[l].description){
                    ...
                    //                                            <span style="font-weight:bold;"><i class="fas fa-check-circle" style="color:#4f9c64;"></i> Reschedule</span>`;
                            ...
                            //                                    <br/><span style="font-weight:bold;"><i class="fas fa-check-circle" style="color:#4f9c64;"></i> Refund</span>`;
                                            ...
                                            if(k != price_itinerary_temp[i].journeys[j].segments.length-1){
                                                text_modal_content+=`<div class="col-lg-12 mb-3" style="border-top:1px solid #cdcdcd;"></div>`;
                                            }
                                        }
                                    text_modal_content+=`
                                    </div>
                            ...
                            text += `
                            <div class="col-lg-12">
                                <div class="span_link" style="display:inline-flex; margin-top:10px;" onclick="content_modal_custom('myModalDetail','myModalContent', 'Detail', modal_content_arr[`+parseInt(flight_count-1)+`]);">
                                    <div>`;
                                        if(!is_roundtrip_combo)
                                            text+=`Flight #`+flight_count+`<br/>`;
                                        else
                                            text+=`Roundtrip</br>`;
                                        text+=price_itinerary_temp[i].journeys[j].origin;
                                        if(!is_roundtrip_combo)
                                        ...
                                        <span id="flight_title_up`+flight_count+`" style="font-weight:bold; color:`+color+`; cursor:pointer;"> Detail</span>
                                    </div>
                                </div>
                            </div>`;
                                                        ...
                                                        <div class="col-lg-12">
                                                            ...
                                                            <div class="row">
                                                                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6" style="text-align:left;">
                                                                    <span style="font-size:13px; font-weight:500;"><b>`+pax_count+`x</b> Fare @ `+airline_price[price_counter][pax_type].currency +' '+getrupiah(airline_price[price_counter][pax_type].fare)+`</span><br/>
                                                                </div>
                                                                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6" style="text-align:right;">
                                                                    <span style="font-size:13px; font-weight:500;">`+airline_price[price_counter][pax_type].currency+` `+getrupiah(airline_price[price_counter][pax_type].fare * pax_count)+`</span>
                                                                </div>
                                                                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6" style="text-align:left;">
                                                                    <span style="font-size:13px; font-weight:500;">Tax & Charges</span>
                                                                </div>
                                                                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6" style="text-align:right;">
                                                                    <span style="font-size:13px; font-weight:500;">`+airline_price[price_counter][pax_type].currency+` `+getrupiah(price)+`</span>
                                                                </div>
                                                            </div>
                                                        </div>
                                                   </div>`;
                                                    $text += pax_count + ' '+pax_type_name+' @'+ airline_price[price_counter][pax_type].currency +' '+getrupiah(airline_price[price_counter][pax_type].fare + (price/pax_count))+'\n';
        //                                                $text += 'Adult Tax '+ airline_price[price_counter].ADT.currency +' '+getrupiah(price)+'\n';
                                                    sub_total_count+=(airline_price[price_counter][pax_type].fare * pax_count) + price;
                                                    price = 0;
        ...
        <div class="row">
            ...
            <div class="col-lg-6 col-md-6" style="text-align:right;">
                <label id="additional_price">`+currency+` `+getrupiah(additional_price)+`</label>
            </div>`;
                ...
                if(passengers.hasOwnProperty('adult')){
                    for(x in passengers['adult']){
                        ...
                        if(passengers['adult'][x].identity_type){
                            $text += '\n- ' + passengers['adult'][x].identity_type.substr(0,1).toUpperCase() + passengers['adult'][x].identity_type.substr(1,passengers['adult'][x].identity_type.length).toLowerCase() + ': ';
                            $text += passengers['adult'][x].identity_number;
...
//                            $text += passengers['adult'][x].identity_number.substr(passengers['adult'][x].identity_number.length-3,passengers['adult'][x].identity_number.length)
                        ...
                        for(y in passengers['adult'][x].ff_numbers){
                            ...
                            $text += passengers['adult'][x].ff_numbers[y].ff_number;
...
//                            $text += passengers['adult'][x].ff_numbers[y].ff_number.substr(passengers['adult'][x].ff_numbers[y].ff_number.length-3,passengers['adult'][x].ff_numbers[y].ff_number.length);
                        ...
                        if(passengers['adult'][x].hasOwnProperty('ssr_list')){
                            for(y in passengers['adult'][x].ssr_list){
                                $text += '\n- ' + passengers['adult'][x].ssr_list[y].origin + '-' + passengers['adult'][x].ssr_list[y].destination + ' ';
                        ...
                        if(passengers['adult'][x].hasOwnProperty('seat_list')){
                            for(y in passengers['adult'][x].seat_list){
                                if(passengers['adult'][x].seat_list[y].seat_name){
                                    $text += '\n- ' + passengers['adult'][x].seat_list[y].segment_code + ' Seat: ' + passengers['adult'][x].seat_list[y].seat_pick;
                                }
                            }
                        }
                    }
                ...
                if(passengers.hasOwnProperty('child')){
                    for(x in passengers['child']){
                        ...
                        if(passengers['child'][x].identity_type){
                            ...
                            $text += passengers['child'][x].identity_number;
...
//                            $text += passengers['child'][x].identity_number.substr(passengers['child'][x].identity_number.length-3,passengers['child'][x].identity_number.length)
                        ...
                        for(y in passengers['child'][x].ff_numbers){
                            ...
                            $text += passengers['child'][x].ff_numbers[y].ff_number;
...
//                            $text += passengers['child'][x].ff_numbers[y].ff_number.substr(passengers['child'][x].ff_numbers[y].ff_number.length-3,passengers['child'][x].ff_numbers[y].ff_number.length);
                        ...
                        if(passengers['child'][x].hasOwnProperty('ssr_list')){
                            for(y in passengers['child'][x].ssr_list){
                                $text += '\n- ' + passengers['child'][x].ssr_list[y].origin + '-' + passengers['child'][x].ssr_list[y].destination + ' ';
                        ...
                        if(passengers['child'][x].hasOwnProperty('seat_list')){
                            for(y in passengers['child'][x].seat_list){
                                if(passengers['child'][x].seat_list[y].seat_name){
                                    $text += '\n- ' + passengers['child'][x].seat_list[y].segment_code + ' Seat: ' + passengers['child'][x].seat_list[y].seat_pick;
                                }
                            }
                        }
                    }
                ...
                if(passengers.hasOwnProperty('student')){
                    for(x in passengers['student']){
                        ...
                        if(passengers['student'][x].identity_type){
                            $text += '\n- ' + passengers['student'][x].identity_type.substr(0,1).toUpperCase() + passengers['student'][x].identity_type.substr(1,passengers['student'][x].identity_type.length).toLowerCase() + ': ';
                            $text += passengers['student'][x].identity_number;
...
//                            $text += passengers['student'][x].identity_number.substr(passengers['student'][x].identity_number.length-3,passengers['student'][x].identity_number.length)
                        ...
                        for(y in passengers['student'][x].ff_numbers){
                            ...
                            $text += passengers['student'][x].ff_numbers[y].ff_number;
...
//                            $text += passengers['student'][x].ff_numbers[y].ff_number.substr(passengers['student'][x].ff_numbers[y].ff_number.length-3,passengers['student'][x].ff_numbers[y].ff_number.length);
                        ...
                        if(passengers['student'][x].hasOwnProperty('ssr_list')){
                            for(y in passengers['student'][x].ssr_list){
                                $text += '\n- ' + passengers['student'][x].ssr_list[y].origin + '-' + passengers['student'][x].ssr_list[y].destination + ' ';
                        ...
                        if(passengers['student'][x].hasOwnProperty('seat_list')){
                            for(y in passengers['student'][x].seat_list){
                                if(passengers['student'][x].seat_list[y].seat_name){
                                    $text += '\n- ' + passengers['student'][x].seat_list[y].segment_code + ' Seat: ' + passengers['student'][x].seat_list[y].seat_pick;
                                }
                            }
                        }
                    }
                ...
                if(passengers.hasOwnProperty('labour')){
                    for(x in passengers['labour']){
                        ...
                        if(passengers['labour'][x].identity_type){
                            $text += '\n- ' + passengers['labour'][x].identity_type.substr(0,1).toUpperCase() + passengers['labour'][x].identity_type.substr(1,passengers['labour'][x].identity_type.length).toLowerCase() + ': ';
                            $text += passengers['labour'][x].identity_number;
...
//                            $text += passengers['labour'][x].identity_number.substr(passengers['labour'][x].identity_number.length-3,passengers['labour'][x].identity_number.length)
                        ...
                        for(y in passengers['labour'][x].ff_numbers){
                            ...
                            $text += passengers['labour'][x].ff_numbers[y].ff_number;
...
//                            $text += passengers['labour'][x].ff_numbers[y].ff_number.substr(passengers['labour'][x].ff_numbers[y].ff_number.length-3,passengers['labour'][x].ff_numbers[y].ff_number.length);
                        ...
                        if(passengers['labour'][x].hasOwnProperty('ssr_list')){
                            for(y in passengers['labour'][x].ssr_list){
                                $text += '\n- ' + passengers['labour'][x].ssr_list[y].origin + '-' + passengers['labour'][x].ssr_list[y].destination + ' ';
                        ...
                        if(passengers['labour'][x].hasOwnProperty('seat_list')){
                            for(y in passengers['labour'][x].seat_list){
                                if(passengers['labour'][x].seat_list[y].seat_name){
                                    $text += '\n- ' + passengers['labour'][x].seat_list[y].segment_code + ' Seat: ' + passengers['labour'][x].seat_list[y].seat_name;
                                }
                            }
                        }
                    }
                ...
                if(passengers.hasOwnProperty('seaman')){
                    for(x in passengers['seaman']){
                        ...
                        if(passengers['seaman'][x].identity_type){
                            $text += '\n- ' + passengers['seaman'][x].identity_type.substr(0,1).toUpperCase() + passengers['seaman'][x].identity_type.substr(1,passengers['seaman'][x].identity_type.length).toLowerCase() + ': ';
                            $text += passengers['seaman'][x].identity_number;
...
//                            $text += passengers['seaman'][x].identity_number.substr(passengers['seaman'][x].identity_number.length-3,passengers['seaman'][x].identity_number.length)
                        ...
                        for(y in passengers['seaman'][x].ff_numbers){
                            ...
                            $text += passengers['seaman'][x].ff_numbers[y].ff_number;
...
//                            $text += passengers['seaman'][x].ff_numbers[y].ff_number.substr(passengers['seaman'][x].ff_numbers[y].ff_number.length-3,passengers['seaman'][x].ff_numbers[y].ff_number.length);
                        ...
                        if(passengers['seaman'][x].hasOwnProperty('ssr_list')){
                            for(y in passengers['seaman'][x].ssr_list){
                                $text += '\n- ' + passengers['seaman'][x].ssr_list[y].origin + '-' + passengers['seaman'][x].ssr_list[y].destination + ' ';
                        ...
                        if(passengers['seaman'][x].hasOwnProperty('seat_list')){
                            for(y in passengers['seaman'][x].seat_list){
                                if(passengers['seaman'][x].seat_list[y].seat_name){
                                    $text += '\n- ' + passengers['seaman'][x].seat_list[y].segment_code + 'Seat: ' + passengers['seaman'][x].seat_list[y].seat_name;
                                }
                            }
                        }
                    }
            ...
            try{
                if(total_discount != 0){
                    ...
                    <div class="col-lg-6" style="text-align:right;">
                        <label>`+currency+` `+getrupiah(total_discount)+`</label>
                    </div>`;
...
//                grand_total_price += upsell_price;
            ...
            text+=`
            <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6" style="text-align:left;">
                <span style="font-size:16px; font-weight:bold;"><b>Total</b></span>
            </div>
            <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6" style="text-align:right;">
        ...
        for(i in airline_get_detail.provider_bookings){
            for(j in airline_get_detail.provider_bookings[i].journeys){
                text+=`
                <div class="row" style="border-top:1px solid #cdcdcd; padding-top:15px;">
                    <div class="col-lg-12">`;
...
//                        }
                    ...
                    for(k in airline_get_detail.provider_bookings[i].journeys[j].segments){
                        ...
                        <div class="row">
                            ...
                            try{
                                ...
                                text+=`<img data-toggle="tooltip" class="airlines_provider_img" alt="Airline" src="`+static_path_url_server+`/public/airline_logo/`+airline_get_detail.provider_bookings[i].journeys[j].segments[k].carrier_code+`.png"><span> </span>`;
                            }
                            try{
                                if(airline_get_detail.provider_bookings[i].journeys[j].segments[k].carrier_code != airline_get_detail.provider_bookings[i].journeys[j].segments[k].operating_airline_code && airline_get_detail.provider_bookings[i].journeys[j].segments[k].operating_airline_code != ''){
                                    text+=`<span style="margin-top:15px; font-size:13px; font-weight:500;">Operated By `+airline_carriers[airline_get_detail.provider_bookings[i].journeys[j].segments[k].operating_airline_code].name+` (`+airline_get_detail.provider_bookings[i].journeys[j].segments[k].carrier_code+airline_get_detail.provider_bookings[i].journeys[j].segments[k].carrier_number+`)</span><br/>`;
                                }else{
                                    text+=`<span style="margin-top:15px; font-size:13px; font-weight:500;">`+airline_carriers[airline_get_detail.provider_bookings[i].journeys[j].segments[k].carrier_code].name+` (`+airline_get_detail.provider_bookings[i].journeys[j].segments[k].carrier_code+airline_get_detail.provider_bookings[i].journeys[j].segments[k].carrier_number+`)</span><br/>`;
                                }
                            }catch(err){
                                // carrier tidak ketemu di dict
                                if(airline_get_detail.provider_bookings[i].journeys[j].segments[k].carrier_code != airline_get_detail.provider_bookings[i].journeys[j].segments[k].operating_airline_code && airline_get_detail.provider_bookings[i].journeys[j].segments[k].operating_airline_code != ''){
                                    text+=`<span style="margin-top:15px; font-size:13px; font-weight:500;">Operated By`+airline_get_detail.provider_bookings[i].journeys[j].segments[k].operating_airline_code+` (`+airline_get_detail.provider_bookings[i].journeys[j].segments[k].carrier_code+airline_get_detail.provider_bookings[i].journeys[j].segments[k].carrier_number+`)</span><br/>`;
                                }else{
                                    text+=`<span style="margin-top:15px; font-size:13px; font-weight:500;">`+airline_get_detail.provider_bookings[i].journeys[j].segments[k].carrier_code+` (`+airline_get_detail.provider_bookings[i].journeys[j].segments[k].carrier_code+airline_get_detail.provider_bookings[i].journeys[j].segments[k].carrier_number+`)</span><br/>`;
                                }
                            }
                            text+=`</div>`;
                            ...
                            text+=`
                                <div class="col-lg-12">
                                    <div class="row">
                                        <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">
                                            <h6>`+airline_get_detail.provider_bookings[i].journeys[j].segments[k].departure_date.split('  ')[1]+`</h6>
                                            <span>`+airline_get_detail.provider_bookings[i].journeys[j].segments[k].departure_date.split('  ')[0]+`</span><br>
                                            <span style="font-weight:500;">`+airline_get_detail.provider_bookings[i].journeys[j].segments[k].origin_city+` (`+airline_get_detail.provider_bookings[i].journeys[j].segments[k].origin+`)</span><br>
                                        </div>
                                        <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">
                                            <div style="text-align:center; position: absolute; left:-30%;">
                                            ...
                                            <div style="text-align:right">
                                                <h6>`+airline_get_detail.provider_bookings[i].journeys[j].segments[k].arrival_date.split('  ')[1]+`</h6>
                                                <span>`+airline_get_detail.provider_bookings[i].journeys[j].segments[k].arrival_date.split('  ')[0]+`</span><br>
                                                <span style="font-weight:500;">`+airline_get_detail.provider_bookings[i].journeys[j].segments[k].destination_city+` (`+airline_get_detail.provider_bookings[i].journeys[j].segments[k].destination+`)</span><br>
                                            </div>
                                        </div>
                                    </div>
                                </div>
        ...
        <div class="row">
            ...
            <div class="col-lg-6 col-md-6" style="text-align:right;">
                <label>`+currency+` `+getrupiah(additional_price)+`</label>
            </div>
            <div class="col-lg-6 col-md-6" style="text-align:left;">
                <span style="font-size:14px; font-weight:bold;"><b>Total</b></span>
            </div>
            <div class="col-lg-6 col-md-6" style="text-align:right;">
                <span style="font-size:14px; font-weight:bold;" id="total_price"><b>`+currency+` `+getrupiah(total_price+additional_price)+`</b></span>
            </div>`;
        text += `
        ...
        try{
            document.getElementById('additional_price_information_rs').innerHTML = `
                <div class="row">
                    ...
                    <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6" style="text-align:right;">
                        <span style="font-size:13px; font-weight:500;">`+currency+` `+getrupiah(additional_price)+`</span>
                    </div>
                </div>
            `;
            document.getElementById('additional_price_information_rs').hidden = false;
            document.getElementById('total_price_rs').innerHTML = `<b>`+currency+` `+getrupiah(total_price+additional_price)+`</b>`;
        }catch(err){
            console.log(err); // error kalau ada element yg tidak ada
        }
    }
                    ...
                    for(k in price_itinerary_temp[i].journeys[j].segments){
                        for(l in price_itinerary_temp[i].journeys[j].segments[k].fares){
                            for(m in price_itinerary_temp[i].journeys[j].segments[k].fares[l].service_charge_summary){
                                ...
                                price_breakdown['OTT'] += price_itinerary_temp[i].journeys[j].segments[k].fares[l].service_charge_summary[m].total_price_ori;
                                price_breakdown['TOTAL PRICE'] += price_itinerary_temp[i].journeys[j].segments[k].fares[l].service_charge_summary[m].total_price;
                                price_breakdown['NTA AGENT'] += price_itinerary_temp[i].journeys[j].segments[k].fares[l].service_charge_summary[m].total_nta;
                                if(user_login.co_agent_frontend_security.includes('agent_ho'))
                                    price_breakdown['COMMISSION HO'] += price_itinerary_temp[i].journeys[j].segments[k].fares[l].service_charge_summary[m].total_commission_ho * -1;
                            }
                        }
                    }
                }
...
function get_airline_review(){
    ...
    <div class="div_box_default mb-3">
        <div class="row">
            <div class="col-lg-12" style="margin:auto;">
                <h4 class="mb-3"><i class="fas fa-plane"></i> Flight Detail</h4>
            </div>
        </div>`;
        flight_count = 0;
        for(i in airline_pick){
            ...
            for(j in airline_pick[i].price_itinerary){
                ...
                if(airline_pick[i].price_itinerary[j].is_combo_price == true){
                    ...
                    flight_count++;
                    text += `<h5 style="margin-bottom:10px;">Flight #`+flight_count+`</h5>`;
                }
                text+=`</div>`;
                ...
                for(k in airline_pick[i].price_itinerary[j].segments){
                    if(airline_pick[i].price_itinerary[j].journey_type == 'COM'){
                        text += `<div><h5 style="margin-bottom:10px;">Flight `+parseInt(flight_count+1)+`</h5></div>`;
                        flight_count++;
                    }
                        ...
                        <div style="display:inline-flex;">
                            ...
                            try{
                                if(airline_pick[i].price_itinerary[j].segments[k].carrier_code != airline_pick[i].price_itinerary[j].segments[k].operating_airline_code && airline_pick[i].price_itinerary[j].segments[k].operating_airline_code != ''){
                                    text+=`<img data-toggle="tooltip" class="airlines_provider_img" alt="`+airline_carriers[airline_pick[i].price_itinerary[j].segments[k].operating_airline_code].name+`" title="`+airline_carriers[airline_pick[i].price_itinerary[j].segments[k].operating_airline_code].name+`" src="`+static_path_url_server+`/public/airline_logo/`+airline_pick[i].price_itinerary[j].segments[k].operating_airline_code+`.png">`;
                                }else{
                                    text+=`<img data-toggle="tooltip" class="airlines_provider_img" alt="`+airline_carriers[airline_pick[i].price_itinerary[j].segments[k].carrier_code].name+`" title="`+airline_carriers[airline_pick[i].price_itinerary[j].segments[k].carrier_code].name+`" src="`+static_path_url_server+`/public/airline_logo/`+airline_pick[i].price_itinerary[j].segments[k].carrier_code+`.png">`;
                                }
                            }catch(err){
                                // carrier tidak ketemu di dict
                                if(airline_pick[i].price_itinerary[j].segments[k].carrier_code != airline_pick[i].price_itinerary[j].segments[k].operating_airline_code && airline_pick[i].price_itinerary[j].segments[k].operating_airline_code != ''){
                                    text+=`<img data-toggle="tooltip" class="airlines_provider_img" alt="`+airline_pick[i].price_itinerary[j].segments[k].operating_airline_code+`" title="`+airline_pick[i].price_itinerary[j].segments[k].operating_airline_code+`" src="`+static_path_url_server+`/public/airline_logo/`+airline_pick[i].price_itinerary[j].segments[k].operating_airline_code+`.png">`;
                                }else{
                                    text+=`<img data-toggle="tooltip" class="airlines_provider_img" alt="`+airline_pick[i].price_itinerary[j].segments[k].carrier_code+`" title="`+airline_pick[i].price_itinerary[j].segments[k].carrier_code+`" src="`+static_path_url_server+`/public/airline_logo/`+airline_pick[i].price_itinerary[j].segments[k].carrier_code+`.png">`;
                                }
                            }
//                            try{
//                                text+=`<img data-toggle="tooltip" class="airlines_provider_img" alt="`+airline_carriers[airline_pick[i].price_itinerary[j].segments[k].carrier_code].name+`" title="`+airline_carriers[airline_pick[i].price_itinerary[j].segments[k].carrier_code].name+`" src="`+static_path_url_server+`/public/airline_logo/`+airline_pick[i].price_itinerary[j].segments[k].carrier_code+`.png">`;
//                            }catch(err){
//                                text+=`<img data-toggle="tooltip" class="airlines_provider_img" alt="Airline" title="" src="`+static_path_url_server+`/public/airline_logo/`+airline_pick[i].price_itinerary[j].segments[k].carrier_code+`.png">`;
//                            }
                            text+=`
                            </div>
                            <div style="display:inline-block;">`;
                                try{
                                    if(airline_pick[i].price_itinerary[j].segments[k].carrier_code != airline_pick[i].price_itinerary[j].segments[k].operating_airline_code && airline_pick[i].price_itinerary[j].segments[k].operating_airline_code != ''){
                                        text+=`<span style="margin-top:15px; font-size:13px; font-weight:500;">Operated By `+airline_carriers[airline_pick[i].price_itinerary[j].segments[k].operating_airline_code].name+` (`+airline_pick[i].price_itinerary[j].segments[k].carrier_code+airline_pick[i].price_itinerary[j].segments[k].carrier_number+`)</span><br/>`;
                                    }else{
                                    ...
                                    // carrier tidak ketemu di dict
                                    if(airline_pick[i].price_itinerary[j].segments[k].carrier_code != airline_pick[i].price_itinerary[j].segments[k].operating_airline_code && airline_pick[i].price_itinerary[j].segments[k].operating_airline_code != ''){
                                        text+=`<span style="margin-top:15px; font-size:13px; font-weight:500;">Operated By`+airline_pick[i].price_itinerary[j].segments[k].operating_airline_code+` (`+airline_pick[i].price_itinerary[j].segments[k].carrier_code+airline_pick[i].price_itinerary[j].segments[k].carrier_number+`)</span><br/>`;
                                    }else{
...
//                            </div>
                    ...
                    for(l in airline_pick[i].price_itinerary[j].segments[k].legs){
                        ...
                        <div class="row">
                            <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">
                                <h6>`+airline_pick[i].price_itinerary[j].segments[k].legs[l].departure_date.split(' - ')[1]+`</h6>
                                <span>`+airline_pick[i].price_itinerary[j].segments[k].legs[l].departure_date.split(' - ')[0]+`</span><br>
                                <span style="font-weight:500;">`+airline_pick[i].price_itinerary[j].segments[k].legs[l].origin_city+` (`+airline_pick[i].price_itinerary[j].segments[k].legs[l].origin+`)</span><br>`;
                                if(airline_pick[i].price_itinerary[j].segments[k].origin_terminal != ''){
                                    text+=`<span style="font-size:13px; font-weight:500;">Terminal: `+airline_pick[i].price_itinerary[j].segments[k].origin_terminal+`</span><br/>`;
                                }else{
                                    text+=`<span style="font-size:13px; font-weight:500;">Terminal: - </span><br/>`;
                                }
                                text+=`
                            ...
                            <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">
                                ...
                                <div style="text-align:right">
                                    <h6>`+airline_pick[i].price_itinerary[j].segments[k].legs[l].arrival_date.split(' - ')[1]+`</h6>
                                    <span>`+airline_pick[i].price_itinerary[j].segments[k].legs[l].arrival_date.split(' - ')[0]+`</span><br>
                                    <span style="font-weight:500;">`+airline_pick[i].price_itinerary[j].segments[k].legs[l].destination_city+` (`+airline_pick[i].price_itinerary[j].segments[k].legs[l].destination+`)</span><br>`;
                                    if(airline_pick[i].price_itinerary[j].segments[k].destination_terminal != ''){
                                        text+=`<span style="font-size:13px; font-weight:500;">Terminal: `+airline_pick[i].price_itinerary[j].segments[k].destination_terminal+`</span><br/>`;
                                    }else{
                                        text+=`<span style="font-size:13px; font-weight:500;">Terminal: - </span><br/>`;
                                    }
                                    text+=`
                    ...
                    text+=`</div>`;
                    text+=`<div class="col-lg-12 mt-3"></div>`;
                }
...
//                            }
                        ...
                        <div style="margin-left:10px;">
                            ...
                            <b>Phone: </b><i>`+passengers.contact[i].calling_code+` - `+passengers.contact[i].mobile+`</i>
                        </div>`;
                    }
                    text+=`
                </div>
    ...
    <div class="row">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <h4 style="margin-bottom:10px;">
                            ...
                            Passengers
                            <button type="button" class="primary-btn hold-seat-booking-train ld-ext-right" style="margin-left:10px; line-height:35px; width:unset; margin-bottom:unset;" id="btn-search-train" onclick="window.location.href = '/airline/passenger/`+signature+`';">
                            ...
                            <div class="row">
                                <div class="col-lg-12" style="background: aliceblue;">
                                    <h5 style="padding-top:10px;">
                                        Passenger #`+(parseInt(count_pax)+1)+`
                                    </h5>
        ...
        //                                }
                                            if(passengers_ssr[i].title == "MR"){
                                                text+=`<img src="/static/tt_website/images/icon/symbol/user_mr.png" alt="User MR" class="picture_passenger_small">`;
                                            }
                                            else if(passengers_ssr[i].title == "MRS"){
                                                text+=`<img src="/static/tt_website/images/icon/symbol/user_mrs.png" alt="User MRS" class="picture_passenger_small">`;
                                            }
                                            else if(passengers_ssr[i].title == "MS"){
                                                text+=`<img src="/static/tt_website/images/icon/symbol/user_ms.png" alt="User MS" class="picture_passenger_small">`;
                                            }
                                            else if(passengers_ssr[i].title == "MSTR"){
                                                text+=`<img src="/static/tt_website/images/icon/symbol/user_mistr.png" alt="User MSTR" class="picture_passenger_small">`;
                                            }
                                        ...
                                        <div style="margin-left:10px;">
                                            ...
                                            if(passengers_ssr[i].identity_type){
                                                text+=`<b>`+passengers_ssr[i].identity_type.substr(0,1).toUpperCase()+passengers_ssr[i].identity_type.substr(1,passengers_ssr[i].identity_type.length)+`</b>: <i>`+passengers_ssr[i].identity_number+`</i>`;
                                            }
                                            if(passengers_ssr[i].identity_expdate){
                                                text+=` | <b>Expired Date</b>: <i>`+passengers_ssr[i].identity_expdate+`</i>`;
                                            }
                                            if(passengers_ssr[i].identity_type || passengers_ssr[i].identity_expdate){
                                                text+=`<br/>`;
                                            }
                                ...
                                <div class="col-lg-12" style="padding:15px;">
                                    <h5 class="mb-3" style="border-bottom: 2px solid #cdcdcd; width:fit-content;">Seat & SSR</h5>
                                    <div id="included_ssr`+i_id+`">
...
//                                        text+=`<h6 class="pt-2" style="border-bottom: 2px solid `+color+`; color: `+color+`; width:fit-content;">SSR additional request</h6>
//                                        <div style="margin-bottom:10px;">`;
//                                    }
//                                    counter_fee_dict = 0;
//                                    for(j in fee_dict){
...
//                                            text += `<b>`;
//                                        }
//                                        text+=`<b>`+fee_dict[j].origin+` - `+fee_dict[j].destination+` (`+fee_dict[j].departure_date+`)</b><br/>
...
//                                                text+=` • <i class="fas fa-suitcase"></i> <i>Added</i> `;
//                                            }
...
//                                        text+=`<h6 class="pt-2" style="border-bottom: 2px solid `+color+`; color: `+color+`;">Seat request</h6>
//                                        <div>`;
//                                    }
//
//                                    counter_seat_list = 0;
...
//                                            text+= `
//                                            <b>`+passengers_ssr[i].seat_list[j].segment_code+` (`+moment(passengers_ssr[i].seat_list[j].departure_date).format('DD MMM YYYY') +`)</b> : <i>` +passengers_ssr[i].seat_list[j].seat_pick + '</i><br>';
//                                        }
//                                    }
//                                    if(passengers_ssr[i].hasOwnProperty('seat_list')){
//                                        text+=`</div>`;
...
//                                </div>`;
                    ...
                    for(i in passengers.infant){
                        if(i != passengers.infant.length-1){
                            text+=`<div class="col-lg-12 mb-3" style="padding-top:15px; border-top:1px solid #cdcdcd; background:aliceblue;">`;
                        }else{
                            text+=`<div class="col-lg-12" style="padding-top:15px; border-top:1px solid #cdcdcd; background:aliceblue;">`;
                        }
...
//                                    text+=`<img src="`+passengers_ssr[i].face_image[0]+`" alt="User" class="picture_passenger_agent">`;
//                                }
                                    if(passengers.infant[i].title == "MSTR"){
                                        text+=`<img src="/static/tt_website/images/icon/symbol/user_mistr.png" alt="User MSTR" class="picture_passenger_small">`;
                                    }
                                ...
                                <div style="margin-left:10px;">
                                    ...
                                    if(passengers.infant[i].identity_type)
                                        text+=`<b>`+passengers.infant[i].identity_type.substr(0,1).toUpperCase()+passengers.infant[i].identity_type.substr(1,passengers.infant[i].identity_type.length)+`</b>: <i>`+passengers.infant[i].identity_number+`</i>`;
                                    text+=`
                                </div>
                            </div>
                        </div>`;
...
function get_airline_review_after_sales(){
    ...
    <div class="div_box_default mb-3">
        <div class="row">
            <div class="col-lg-12">
                <h4 class="mb-3"><img src="/static/tt_website/images/icon/product/b-airline.png" alt="Airline" style="width:20px; height:20px;"> Flight Detail</h4>
            </div>
        </div>`;
    flight_count = 0;
    for(i in airline_get_detail.provider_bookings){
...
//                                }
                    ...
                    <div class="col-lg-12" style="padding:15px;">
                        ...
                        if(Object.keys(airline_get_detail.passengers[i].fee_dict).length != 0){
                            text+=`<div class="row">`;
                            ...
                            for(j in airline_get_detail.passengers[i].fee_dict){
                                ...
                                <div class="col-lg-12" style="margin-bottom:15px;">
                                    <h6 style="margin-bottom:10px;">`+airline_get_detail.passengers[i].fee_dict[j].origin+` <i class="fas fa-arrow-right"></i> `+airline_get_detail.passengers[i].fee_dict[j].destination+` ( `+airline_get_detail.passengers[i].fee_dict[j].departure_date+` )</h6>`;

                                    for(k in airline_get_detail.passengers[i].fee_dict[j].fees){
                                        text+=`• <b style="padding:5px; margin-right:5px;">(Current Request)</b>`
                                        ...
                                        if(airline_get_detail.passengers[i].fee_dict[j].fees[k].fee_name.toLowerCase().includes(airline_get_detail.passengers[i].fee_dict[j].fees[k].fee_category.toLowerCase()) == false)
                                            text += `<i style="text-transform: capitalize;">`+airline_get_detail.passengers[i].fee_dict[j].fees[k].fee_category+`</i> `;
                                        text += airline_get_detail.passengers[i].fee_dict[j].fees[k].fee_name + `</i><br/>`;
                                    }

                                    try{
                                        for(k2 in passengers_ssr[i].fee_dict[j].fees){
                                            text+=`• <b style="color:`+color+`; padding:5px; margin-right:5px;">(Change SSR Request`;
                                            if(passengers_ssr[i].fee_dict[j].fees[k2].fee_category == 'baggage'){
                                            ...
                                            text+=`)</b>`;
                                            if(passengers_ssr[i].fee_dict[j].fees[k2].fee_category == 'meal'){
                                                text+=`<i class="fas fa-utensils"></i> `;
                                            }
                                            else if(passengers_ssr[i].fee_dict[j].fees[k2].fee_category == 'baggage'){
                                                text+=`<i class="fas fa-suitcase"></i> `;
                                            }
                                            else if(passengers_ssr[i].fee_dict[j].fees[k2].fee_category == 'wheelchair'){
                                                text+=`<i class="fas fa-wheelchair"></i> `;
                                            }
                                            else if(passengers_ssr[i].fee_dict[j].fees[k2].fee_category == 'equipment'){
                                                text+=`<i class="fas fa-tools"></i> `;
                                            }
                                            else if(passengers_ssr[i].fee_dict[j].fees[k2].fee_category == 'seat'){
                                                text+=`<img src="/static/tt_website/images/icon/symbol/seat.png" style="height:16px; width:auto;"/> `;
                                            }

                                            text+=`<i>`;
                                            if(passengers_ssr[i].fee_dict[j].fees[k2].fee_name.toLowerCase().includes(passengers_ssr[i].fee_dict[j].fees[k2].fee_category.toLowerCase()) == false)
                                                text += passengers_ssr[i].fee_dict[j].fees[k2].fee_category + ': ';
                                            text += passengers_ssr[i].fee_dict[j].fees[k2].fee_name + `</i><br/>`;
                                        }
                                    }catch(err){
                                          console.log(err); // error kalau ada element yg tidak ada
                                    }
                                    ...
                                    if(typeof passengers_ssr[i].seat_list !== 'undefined'){
                                        try{
                                            if(passengers_ssr[i].seat_list[counter_req].seat_pick != ''){
                                                text += `
                                                • <b style="color:`+color+`; padding:5px;">(Change Seat Request)</b>
                                                <img src="/static/tt_website/images/icon/symbol/seat.png" style="height:16px; width:auto;"/>
                                                <i>Seat `+passengers_ssr[i].seat_list[counter_req].seat_pick + `</i>`;
                                            }
                                        }catch(err){
                                            console.log(err); // error kalau ada element yg tidak ada
                                        }
                ...
                //                                        text += `<label>`+airline_get_detail.passengers[i].fees[j].fee_name+ ' ' + airline_get_detail.passengers[i].fees[j].fee_value + `</label><br/>`;
                            ...
                            text+=`<div class="row">`;
                            if(Object.keys(passengers_ssr[i].fee_dict).length != 0){
                                for(j in passengers_ssr[i].fee_dict){
                                    journey_key = j;
                                    text += `
                                    <div class="col-lg-12" style="margin-bottom:15px;">
                                        <h6 style="margin-bottom:10px;">`+passengers_ssr[i].fee_dict[j].origin+` <i class="fas fa-arrow-right"></i> `+passengers_ssr[i].fee_dict[j].destination+` ( `+passengers_ssr[i].fee_dict[j].departure_date+` )</h6>
                                        • <b style="padding:5px;">(No current request)</b><br/>`;

                                        for(k in passengers_ssr[i].fee_dict[j].fees){
                                            text+=`• <b style="color:`+color+`; padding:5px; margin-right:5px;">(New Request`;
                    ...
                    //                                        text += `<label>`+passengers_ssr[i].ssr_list[j].availability_type+ ' ' + passengers_ssr[i].ssr_list[j].name + `</label><br/>`;
                                ...
                                text+=`
                                <div class="col-lg-12" style="margin-bottom:15px;">
                                    • <b style="padding:5px;">(No current request)</b>
                                </div>`;
                            }
                            text+=`</div>`;
                        }
...
function change_date_next_prev(counter){
    var today_date = moment().format('ddd, DD MMM YYYY'); //hari ini
    ...
    <div class="owl-carousel-change-search owl-theme">
    ...
    $('.owl-carousel-change-search').owlCarousel({
        loop:false,
        nav: true,
        margin: 20,
    ...
    document.getElementById('now_date').innerHTML = `<div class="button_date_np" style="background:#f7f7f7; border:2px solid `+color+`; text-align: center; box-shadow: rgb(3 18 26 / 15%) 4px -1px 10px -4px, rgb(3 18 26 / 15%) -4px -1px 10px -4px;" id="div_onclick_now_date">`+flight_date.format(date_format)+`</div>`;
            ...
            for(i in itinerary){
                ...
                for(j in itinerary[i].journeys){
                    for(k in itinerary[i].journeys[j].segments){
                        default_ssr_text+=`<h6 style="margin-bottom:10px;">`+itinerary[i].journeys[j].segments[k].origin+` <i class="fas fa-arrow-right"></i> `+itinerary[i].journeys[j].segments[k].destination+` ( `+itinerary[i].journeys[j].segments[k].departure_date+` )</h6>
                        <div style="padding:0px 15px;">`;
                        if(page == 'booking'){
                            ...
                            default_ssr_text += `
                            <div class="row">
                                <div class="col-lg-12" id="div_request_new_ssr`+id_co+``+i+``+j+``+k+`">

                                </div>
                                <div class="col-lg-12" id="div_request_new_seat`+id_co+``+i+``+j+``+k+`">

                                </div>
                            </div>`;
                        }
                        default_ssr_text+=`</div>`;
                    }
                }
            }
...
function auto_fill_airline_cookie(cookie_airline,page='home', max_try=0){
    if(cookie_airline){
        ...
        if(['RT', 'MC'].includes(cookie_airline['direction'])){
            if(cookie_airline['direction'] == 'RT'){
                data_direction = 'roundtrip'
                $('#span_radio_airline_type').text('Round Trip');
            }else{
                data_direction = 'multicity'
                $('#span_radio_airline_type').text('Multi City');
            }
            $("input[name=radio_airline_type][value='"+data_direction+"']").prop("checked",true);
