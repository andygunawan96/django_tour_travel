           ...
           for(k in airline.schedules[i].journeys[j].segments){
               ...
               for(l in airline.schedules[i].journeys[j].segments[k].fares){
                   ...
                   if(airline.schedules[i].journeys[j].segments[k].fares[l].available_count >= parseInt(airline_request.adult)+parseInt(airline_request.child) || airline.schedules[i].journeys[j].segments[k].fares[l].available_count == -1){//atau buat sia
                       ...
                       if(has_fare_details && airline.schedules[i].journeys[j].segments[k].fares[l].fare_details.length > 0){
                            if(fare_details.length > 0){
                                ...
                                for(m=0;m<fare_details.length;m++){
                                    has_same_fare_detail_category = false;
                                    for(n in airline.schedules[i].journeys[j].segments[k].fares[l].fare_details){
                                        if(fare_details[m].detail_type == airline.schedules[i].journeys[j].segments[k].fares[l].fare_details[n].detail_type){
                                            has_same_fare_detail_category = true;
                                        }
                                    }
                                    if(has_same_fare_detail_category == false){
                                        fare_details_list_to_delete.push(m)
                                    }
                                }
                            ...
                            for(m in airline.schedules[i].journeys[j].segments[k].fares[l].fare_details){
                                ...
                                for(n in fare_details){
                                    ...
                                    }else if(fare_details[n].amount > airline.schedules[i].journeys[j].segments[k].fares[l].fare_details[m].amount && fare_details[n].detail_type == airline.schedules[i].journeys[j].segments[k].fares[l].fare_details[m].detail_type){
                                        fare_details.splice(n,1);
                                        break;
                                    }else if(k != 0 && fare_details[n].detail_type == airline.schedules[i].journeys[j].segments[k].fares[l].fare_details[m].detail_type){
...
//                                    $text +='Flight '+flight_count+'\n';
                            ...
                            //                            $text += resJson.result.response.price_itinerary_provider[i].journeys[j].segments[k].destination_name + ' (' + resJson.result.response.price_itinerary_provider[i].journeys[j].segments[k].destination_city + ') '+resJson.result.response.price_itinerary_provider[i].journeys[j].segments[k].arrival_date +'\n\n';
                                                        ...
                                                        if(resJson.result.response.price_itinerary_provider[i].journeys[j].segments[k].fares.length > 0){
                                                            text += `<img src="/static/tt_website_rodextrip/img/icon/seat.png" style="height:16px; width:auto;"><span style="color:`+color+`; font-weight:800;">`;
...
function render_price_in_get_price(text, $text, $text_share){
                ...
                <div style="padding:15px; border-right:1px solid #cdcdcd; border-left:1px solid #cdcdcd;">
                    <div class="row">
                        ...
                        <div class="col-lg-6 col-xs-6" style="text-align:right;">
                            <span id="span-subtotal-down`+subtotal_div+`" style="color:`+color+`; display:block; font-weight:bold; cursor:pointer;" onclick="show_hide_subtotal(`+subtotal_div+`)">Hide Detail <i class="fas fa-chevron-down"></i></span>
                        </div>
                    </div>
                </div>
            </div>`;
                ...
                <div class="col-lg-12">
                    <div style="padding:15px; border-right:1px solid #cdcdcd; border-left:1px solid #cdcdcd;">
                        <div class="row">
                            ...
                            <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6" style="text-align:right;">
                                <span style="font-size:13px; color:#e04022; font-weight:500;">`+currency+` `+getrupiah(Math.ceil(discount))+`</span>
                            </div>
                        </div>
                    </div>
                </div>`;
    ...
    sub_total_count += Math.ceil(discount);
                                ...
                                for(m in seat_map.seat_availability_provider[i].segments[j].seat_cabins[k].seat_rows[l].seats){
                                    ...
                                    if(check == 0){
                                        if(seat_map.seat_availability_provider[i].segments[j].seat_cabins[k].seat_rows[l].seats[m].availability == 0){
                                            ...
                                            text+=`<input type="button" class="button-seat-map" style="width:`+percent+`%;background-color:transparent;font-size:13px;border:transparent; padding:3px;color:white;" id="`+seat_map.seat_availability_provider[i].segments[j].segment_code2+`_`+seat_map.seat_availability_provider[i].segments[j].departure_date+`_`+seat_map.seat_availability_provider[i].segments[j].seat_cabins[k].seat_rows[l].row_number+`_`+seat_map.seat_availability_provider[i].segments[j].seat_cabins[k].seat_rows[l].seats[m].column+`" value="`+seat_map.seat_availability_provider[i].segments[j].seat_cabins[k].seat_rows[l].row_number+`A" disabled/>`;
                                        }
                                    }
                                }
                                text+=`</div>`;
...
function set_passenger_seat_map_airline(val){
    ...
    for(i in passengers[val].seat_list){
        ...
        if(passengers[val].seat_list[i].seat_code != '')
       ...
       success: function(msg) {
           if(msg.result.error_code == 0){
                ...
                if(error_log == '')
                    ...
                    Swal.fire({
                      title: error_log + ', Do you want to continue this book without Seat?',
                      type: 'warning',
                      showCancelButton: true,
                      confirmButtonColor: '#3085d6',
                      cancelButtonColor: '#d33',
...
function re_order_set_airline_request(type='reorder'){
    $.ajax({
       ...
       error: function(XMLHttpRequest, textStatus, errorThrown) {
            please_wait_custom('Set Request Failed <i class="fas fa-times-circle" style="color:#f53e31"></i>');
...
function re_order_set_passengers(type){
    $.ajax({
       ...
       success: function(resJson) {
            if(type == 'reorder'){
                please_wait_custom('Set Passenger <i class="fas fa-check-circle" style="color:'+color+';"></i><br/>Select Data Journey, please wait <img src="/static/tt_website_rodextrip/img/loading-dot-white.gif" style="height:50px; width:50px;"/>');
    ...
    $.ajax({
       ...
       success: function(msg) {
            if(msg.result.error_code == 0){
                ...
                setTimeout(function(){
                    please_wait_custom('Get Price <i class="fas fa-check-circle" style="color:'+color+';"></i><br/>Sell Journey, please wait <img src="/static/tt_website_rodextrip/img/loading-dot-white.gif" style="height:50px; width:50px;"/>');
                    re_order_get_fare_rules();
                }, 1000);
            }else{
                please_wait_custom('Get Price Failed <i class="fas fa-times-circle" style="color:#f53e31"></i>');
                $('.next-loading-reorder').removeClass("running");
                $('.next-loading-reorder').prop('disabled', false);
                $('.issued_booking_btn').prop('disabled', false);
                $('#button-sync-status').prop('disabled', false);
                    ...
                    if(msg.result.response.reschedule_list.length>0){
                        ...
                        <div style="background-color:white; border:1px solid #cdcdcd;margin-top:20px;">
                            <div class="row">
                                <div class="col-lg-12">
                                    <div style="padding:10px; background-color:white;">
                                    <h5> History <img style="width:18px;" src="/static/tt_website_rodextrip/images/icon/plane.png" alt="Reschedule Flight Detail"/></h5>
                                    <hr/>`;
                                check = 0;
                                for(i in msg.result.response.reschedule_list){
                                    ...
                                    text+=`<h5>`+reschedule_type.substr(0,1).toUpperCase()+reschedule_type.substr(1,reschedule_type.length)+`: `+msg.result.response.reschedule_list[i].reschedule_number+`</h5>`;
                                    text+=`</div>
                                           <div class="col-lg-6" style="text-align:right;">`
                                    text+=`<h5>`;
                                    ...
                                    if(msg.result.response.reschedule_list[i].old_segments.length > 0){
                                        ...
                                        text+=`<h4 style="color:`+color+`">New</h4>`;
...
function render_ticket_reissue(){
    ...
    if(airline.length > 0){
        ...
        try{
            document.getElementById("reorder_div").innerHTML = '';
            document.getElementById("alert-state").style.display = 'none';
        }catch(err){
            console.log(err);
        }
        if(document.getElementById('cancel'))
            document.getElementById('cancel').hidden = true;
        if(document.getElementById('split_booking'))
            document.getElementById('split_booking').hidden = true;
...
//                text_detail+=`
//                <div style="margin-bottom:5px;">
//                    <input class="primary-btn-ticket" id="show_commission_button" style="width:100%;" type="button" onclick="show_commission('commission');" value="Hide YPM"/>
//                </div>
//            </div>`;
//            }catch(err){}
//            document.getElementById('airline_detail').innerHTML = text_detail;
